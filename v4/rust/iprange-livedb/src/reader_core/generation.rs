//! Borrowed logical access to one selected mapped generation.
...
use crate::immutable_output::OutputSpec;
...
/// Logical read capability over a selected generation owned elsewhere.
#[derive(Clone, Copy)]
pub(crate) struct GenerationReader<'a> {
    mapping: &'a Mapping,
    meta: MetaV4,
    owner_identity: Option<ProcessIdentity>,
}

impl<'a> GenerationReader<'a> {
    ...
    pub(crate) fn output_spec(self) -> OutputSpec {
        OutputSpec {
            ...
            commit_nonce: self.meta.commit_nonce,
            feed_index_limit: self.meta.feed_index_limit,
        }
    }

    pub(crate) fn lookup_direct_v4(self, address: Ipv4Key) -> Result<Option<u32>> {
        self.require_direct(AddressFamily::Ipv4)?;
        range_tree::lookup(self.mapping, &self.meta, address)
    }
    ...
    pub(crate) fn direct_cursor_v4(self, direction: RangeDirection) -> Result<DirectCursorV4<'a>> {
        self.require_direct(AddressFamily::Ipv4)?;
        DirectCursorV4::new(self.mapping, &self.meta, direction, self.owner_identity)
    }

    pub(crate) fn direct_cursor_v6(self, direction: RangeDirection) -> Result<DirectCursorV6<'a>> {
        self.require_direct(AddressFamily::Ipv6)?;
        DirectCursorV6::new(self.mapping, &self.meta, direction, self.owner_identity)
    }
    ...
    pub(crate) fn lookup_membership_v4(
        ...
        membership_view::lookup_v4(self.mapping, &self.meta, address, self.owner_identity)
    }
    ...
    pub(crate) fn membership(self, token: MembershipToken) -> Result<MembershipView<'a>> {
        membership_view::by_id(self.mapping, &self.meta, token.id(), self.owner_identity)
    }

    pub(crate) fn metadata_json_len(self) -> Option<u64> {
        (self.meta.metadata_root != 0).then_some(self.meta.metadata_uncompressed_len)
    }

    pub(crate) fn membership_entry_count(self) -> u64 {
        self.meta.membership_entry_count
    }

    pub(crate) fn read_metadata_json(self, output: &mut [u8]) -> Result<Option<usize>> {
        metadata::read(self.mapping, &self.meta, output)
    }

    pub(crate) fn metadata_json(self) -> Result<Option<Vec<u8>>> {
        metadata::read_vec(self.mapping, &self.meta)
    }

    pub(crate) fn open_direct_state(self, direction: RangeDirection) -> Result<ReaderCursor> {
        ReaderCursor::direct(self.mapping, &self.meta, direction, self.owner_identity)
    }
    ...
    pub(crate) fn cursor_next(self, cursor: &mut ReaderCursor) -> Result<Option<ReaderCursorItem>> {
