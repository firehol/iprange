...
use crate::artifact_name::{
    decode_attempt, decode_ordinal, write_attempt, write_ordinal, ATTEMPT_HEX_SIZE as ATTEMPT_HEX,
    ORDINAL_HEX_SIZE as ORDINAL_HEX,
};
...
fn encode(prefix: &[u8], attempt: [u8; 16], ordinal: u32) -> Result<Vec<u8>, NamespaceError> {
    ...
    let attempt_at = bytes.len();
    bytes.resize(attempt_at + ATTEMPT_HEX, 0);
    write_attempt(attempt, &mut bytes[attempt_at..]);
    bytes.push(b'-');
    let ordinal_at = bytes.len();
    bytes.resize(ordinal_at + ORDINAL_HEX, 0);
    write_ordinal(ordinal, &mut bytes[ordinal_at..]);
...
fn decode(bytes: &[u8], prefix: &[u8]) -> Option<([u8; 16], u32)> {
    ...
    let attempt = decode_attempt(&encoded[..ATTEMPT_HEX])?;
    if attempt == [0; 16] {
        return None;
    }
    let ordinal = decode_ordinal(&encoded[ATTEMPT_HEX + 1..])?;
