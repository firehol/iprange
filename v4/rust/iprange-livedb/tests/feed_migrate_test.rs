...
fn dr(f: u32, t: u32) -> DesiredRecord<Ipv4Key> {
    DesiredRecord { from: Ipv4Key(f), to: Ipv4Key(t), scope_id: 0 }
}

fn per_ip_map(img: &[u8]) -> BTreeMap<u32, u32> {
    let r = Reader::open(img).unwrap();
    let mut map = BTreeMap::new();
    r.scan_v4(|f, t, s| { for ip in f.0..=t.0 { map.insert(ip, s); } }).unwrap();
    map
}
...
fn feed_migrate_preserve_other_feeds() {
    ...
    for ip in 10..=14 { assert!(has_feed_bit(map[&ip], 0)); assert!(!has_feed_bit(map[&ip], 1)); }
    for ip in 15..=25 { assert!(has_feed_bit(map[&ip], 0)); assert!(has_feed_bit(map[&ip], 1)); }
    for ip in 26..=30 { assert!(has_feed_bit(map[&ip], 0)); assert!(!has_feed_bit(map[&ip], 1)); }
}
...
fn feed_migrate_remove_feed() {
    ...
    for ip in 15..=19 { assert!(has_feed_bit(map[&ip], 0)); assert!(!has_feed_bit(map[&ip], 1)); }
    for ip in 20..=22 { assert!(has_feed_bit(map[&ip], 0)); assert!(has_feed_bit(map[&ip], 1)); }
    for ip in 23..=25 { assert!(has_feed_bit(map[&ip], 0)); assert!(!has_feed_bit(map[&ip], 1)); }
}
