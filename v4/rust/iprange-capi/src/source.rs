...
fn validate_outcome(
    ...
    match outcome {
        BATCH => {
            ...
            if count == 0 || count > capacity {
                return Err(BoundaryError::invalid_length(
                    "callback batch count is outside 1..=capacity",
                )
                .into());
            }
        ...
        END => {
            ...
            if count != 0 {
                return Err(
                    BoundaryError::invalid_length("callback End must return zero records").into(),
                );
            }
            Ok(Outcome::End)
        }
        CALLBACK_ERROR => {
            if count != 0 {
                return Err(BoundaryError::invalid_length(
                    "callback Error must return zero records",
                )
                .into());
            }
