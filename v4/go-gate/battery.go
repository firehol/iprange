...
var batteryCases = []batteryCase{
	...
	{name: "310: unix.Fallocate outside the mapping owner", desc: "raw file-geometry preallocation outside the mapping owner", expectFail: true, expectRule: "banned content-transfer selector .Fallocate", ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_fallocate.go", content: "//go:build linux\npackage reader\n\nimport \"golang.org/x/sys/unix\"\n\nfunc gateFallocate(fd int, n int64) error { return unix.Fallocate(fd, 0, 0, n) }"},
	}},
	...
	{name: "322: nil func-typed callback argument stays legal", desc: "an approved module callee with a func-typed formal may receive nil: nil has no body and cannot launder a complete page", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "zz_gatemut_nilcb.go", content: "package iprangedb\n\nimport (\n\t\"github.com/firehol/iprange/v4/go/internal/format\"\n\t\"github.com/firehol/iprange/v4/go/internal/writer\"\n)\n\n// GateNilCallbackProbe builds an output with a nil checkpoint callback:\n// a nil function value has no body, so the scanned-callback fence must\n// accept it as a clean sentinel.\nfunc GateNilCallbackProbe() error {\n\tspec, err := writer.FreshOutputSpec(format.AddressFamilyIPv4, format.ValueKindMembership, format.StructureKindNone, [16]byte{}, 1)\n\tif err != nil {\n\t\treturn err\n\t}\n\tbuilder, err := writer.NewOutputBuilder(\"/tmp/gate-nil-cb-probe\", spec, writer.OutputBudget{MaxOutputPages: 4}, nil)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn builder.Close()\n}\n"},
	}},

	{name: "323: error value through a scanned local closure stays legal", desc: "a summary-tainted error passed into a never-reassigned local func-literal closure is not an unproven-callee launder: the closure body is part of the scanned file, so the call site is proven", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "zz_gatemut_closureerr.go", content: "package iprangedb\n\nimport \"github.com/firehol/iprange/v4/go/internal/writer\"\n\n// GateClosureErrProbe routes a summary-tainted error through a local\n// closure; the closure body is scanned like any other func literal.\nfunc GateClosureErrProbe(b *writer.OutputBuilder) error {\n\tdiscarded := func(cause error) error { return cause }\n\tif err := b.Finish(); err != nil {\n\t\treturn discarded(err)\n\t}\n\treturn nil\n}\n"},
	}},

	{name: "324: bounded feed name through a method accessor stays legal", desc: "string conversion of a decoded catalog name fetched through the State().Names() accessor keeps the <=255-byte feed-name bound and is not a complete-page copy", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_nameacc.go", content: "package reader\n\n// GateNameAccessorProbe converts a decoded catalog name to a string\n// through the State accessor, whose summary preserves the bounded\n// feed-name provenance (the v4 name grammar caps names below a page).\nfunc GateNameAccessorProbe(a *MembershipAlgebra) string {\n\tentries := a.State().Names()\n\tif len(entries) == 0 {\n\t\treturn \"\"\n\t}\n\treturn string(entries[0].Name)\n}\n"},
	}},
	{name: "325: local func-literal alias chain laundering a full page", desc: "cb := func(p){ copy(out,p) }; cb2 := cb; cb2(page) with a complete mapped page argument: the rules pass previously approved the local alias chain (approvedLocalFuncVar recursion) while the flow pass cannot resolve calls through local aliases (calleeTarget reads only direct literal bindings and package initializers), so the full page reached a copying body undetected (round-1 Peirce P1). Alias chains now fail closed as unproven variable indirections", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_localalias.go", content: "package reader\n\nfunc localAliasProbe325(r *ImmutableReader, pgno uint32) ([]byte, error) {\n\tpage, err := r.page(pgno)\n\tif err != nil {\n\t\treturn nil, err\n\t}\n\tcb := func(p []byte) []byte {\n\t\tout := make([]byte, len(p))\n\t\tcopy(out, p)\n\t\treturn out\n\t}\n\tcb2 := cb\n\treturn cb2(page), nil\n}"},
	}},
	{name: "326: benign local func-literal alias chain with a bounded slice", desc: "the 325 alias chain called with a bounded record slice (page[48:112]) stays legal: the unproven-indirection fence fires only on complete-page arguments", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_localaliasok.go", content: "package reader\n\nfunc localAliasOkProbe326(r *ImmutableReader, pgno uint32) ([]byte, error) {\n\tpage, err := r.page(pgno)\n\tif err != nil {\n\t\treturn nil, err\n\t}\n\tcb := func(p []byte) []byte {\n\t\tout := make([]byte, len(p))\n\t\tcopy(out, p)\n\t\treturn out\n\t}\n\tcb2 := cb\n\treturn cb2(page[48:112]), nil\n}"},
	}},
	{name: "327: local alias of a same-package copying function with a full page", desc: "cb2 := cloneFn; cb2(page) where cloneFn copies its argument into owned memory: the approvedLocalFuncVar identifier branch previously approved local aliases of scanned package functions even though the flow pass cannot resolve them at the call site; both shapes now fail closed (same class as 325)", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_localfnalias.go", content: "package reader\n\nfunc cloneLocalFn327(p []byte) []byte {\n\tout := make([]byte, len(p))\n\tcopy(out, p)\n\treturn out\n}\n\nfunc localFnAliasProbe327(r *ImmutableReader, pgno uint32) ([]byte, error) {\n\tpage, err := r.page(pgno)\n\tif err != nil {\n\t\treturn nil, err\n\t}\n\tcb2 := cloneLocalFn327\n\treturn cb2(page), nil\n}"},
	}},
	{name: "328: benign local alias of a same-package function with a bounded slice", desc: "the 327 alias called with a bounded record slice stays legal", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_localfnaliasok.go", content: "package reader\n\nfunc cloneLocalFn328(p []byte) []byte {\n\tout := make([]byte, len(p))\n\tcopy(out, p)\n\treturn out\n}\n\nfunc localFnAliasOkProbe328(r *ImmutableReader, pgno uint32) ([]byte, error) {\n\tpage, err := r.page(pgno)\n\tif err != nil {\n\t\treturn nil, err\n\t}\n\tcb2 := cloneLocalFn328\n\treturn cb2(page[48:112]), nil\n}"},
	}},
}
