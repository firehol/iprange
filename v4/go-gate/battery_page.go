...
var batteryPageCases = []batteryCase{
	...
	{name: "P137: store CopyPage callback copying between mapped pages stays benign", desc: "CopyPage(src, dst, func(src, output []byte) { copy(output, src) }) with the callback seeded as a mapping alias is the module's COW page-copy shape and must pass", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cowcb.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype cowStoreImpl137 struct{ r *ImmutableReader }\n\nfunc (s cowStoreImpl137) TargetTxn() uint64 { return 0 }\nfunc (s cowStoreImpl137) PageLimit() uint64 { return 0 }\nfunc (s cowStoreImpl137) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s cowStoreImpl137) Allocate() (uint32, error) { return 0, nil }\nfunc (s cowStoreImpl137) Update(uint32, func([]byte) error) error { return nil }\nfunc (s cowStoreImpl137) DiscardPrivate(uint32) error { return nil }\n\nfunc (s cowStoreImpl137) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tsrc, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\tdst, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn fn(src, dst)\n}\n\nfunc cowCbProbe137(s tree.Store, src, dst uint32) error {\n\treturn s.CopyPage(src, dst, func(src, output []byte) error {\n\t\tcopy(output, src)\n\t\treturn nil\n\t})\n}\n"},
	}},
	{name: "P138: store CopyPage callback laundering a mapped page into an owned buffer fails closed", desc: "the callback appending a seeded mapped src into a package-level owned buffer must be rejected at the append site", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cowcb_bad.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype cowStoreBad138 struct{ r *ImmutableReader }\n\nfunc (s cowStoreBad138) TargetTxn() uint64 { return 0 }\nfunc (s cowStoreBad138) PageLimit() uint64 { return 0 }\nfunc (s cowStoreBad138) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s cowStoreBad138) Allocate() (uint32, error) { return 0, nil }\nfunc (s cowStoreBad138) Update(uint32, func([]byte) error) error { return nil }\nfunc (s cowStoreBad138) DiscardPrivate(uint32) error { return nil }\n\nfunc (s cowStoreBad138) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tsrc, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\tdst, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn fn(src, dst)\n}\n\nvar sinkBuf138 []byte\n\nfunc cowCbBadProbe138(s tree.Store, src, dst uint32) error {\n\treturn s.CopyPage(src, dst, func(src, output []byte) error {\n\t\tsinkBuf138 = append(sinkBuf138, src...)\n\t\treturn nil\n\t})\n}\n"},
	}},
	{name: "P242: stdlib-satisfiable module-internal interface dispatch fails closed", desc: "a module-declared interface whose method set a stdlib type satisfies (UnmarshalText([]byte) error on *big.Int) is NOT an approved indirection: the implementation body is out-of-module and unscanned, so a full mapped page argument must be rejected at the dispatch", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_stdlibifc.go", content: "package reader\n\nimport (\n\t\"math/big\"\n)\n\ntype textUnmarshaler242 interface{ UnmarshalText([]byte) error }\n\nfunc stdlibIfaceProbe242(r *ImmutableReader, pgno uint32) error {\n\tpage, err := r.page(pgno)\n\tif err != nil {\n\t\treturn err\n\t}\n\tvar tu textUnmarshaler242 = new(big.Int)\n\treturn tu.UnmarshalText(page)\n}\n"},
	}},

	{name: "P243: store callback invocation with an owned buffer in a dishonest store implementation fails closed", desc: "CopyPage(src, dst, fn) whose implementation calls fn(s.buf, s.buf) with OWNED buffers violates the store contract: the dispatch-site callback seeding treats the literal params as mapping aliases, so a copy of a real mapped page into the callback's output would launder complete pages into owned memory; the implementation-site check must reject the invocation", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_liar.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype liarStore243 struct{ buf []byte }\n\nfunc (s liarStore243) TargetTxn() uint64 { return 0 }\nfunc (s liarStore243) PageLimit() uint64 { return 0 }\nfunc (s liarStore243) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarStore243) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarStore243) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarStore243) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarStore243) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\treturn fn(s.buf, s.buf)\n}\n\nfunc liarProbe243(page []byte) error {\n\tvar st tree.Store = liarStore243{}\n\treturn st.CopyPage(0, 1, func(src, output []byte) error {\n\t\tcopy(output, page)\n\t\treturn nil\n\t})\n}\n"},
	}},
	...
	{name: "P297: store callback forwarded through a helper with owned buffers fails closed", desc: "CopyPage(src, dst, fn) whose implementation calls runCb297(fn, s.buf, s.buf) with a scanned helper forwarding its byte parameters into the callback formal violates the store contract: the dispatch-site callback seeding treats the literal params as mapping aliases, so the helper indirection must not launder owned buffers into the callback", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbfwd.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype liarStore297 struct{ buf []byte }\n\nfunc (s liarStore297) TargetTxn() uint64 { return 0 }\nfunc (s liarStore297) PageLimit() uint64 { return 0 }\nfunc (s liarStore297) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarStore297) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarStore297) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarStore297) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarStore297) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\treturn runCb297(fn, s.buf, s.buf)\n}\n\nfunc runCb297(fn func(src, output []byte) error, src, output []byte) error { return fn(src, output) }\n\nfunc liarProbe297(page []byte) error {\n\tvar st tree.Store = liarStore297{}\n\treturn st.CopyPage(0, 1, func(src, output []byte) error {\n\t\tcopy(output, page)\n\t\treturn nil\n\t})\n}\n"},
	}},

	{name: "P298 benign: store callback forwarded through a helper with mapped views passes", desc: "CopyPage(src, dst, fn) whose implementation sources mapped views from s.r.page and forwards them through runCb298: the store contract is satisfied, and the scanned helper indirection must not reject the honest forwarding", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbfwd_ok.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype honestStore298 struct{ r *ImmutableReader }\n\nfunc (s honestStore298) TargetTxn() uint64 { return 0 }\nfunc (s honestStore298) PageLimit() uint64 { return 0 }\nfunc (s honestStore298) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s honestStore298) Allocate() (uint32, error) { return 0, nil }\nfunc (s honestStore298) Update(uint32, func([]byte) error) error { return nil }\nfunc (s honestStore298) DiscardPrivate(uint32) error { return nil }\n\nfunc (s honestStore298) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tsrcPage, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\tdstPage, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn runCb298(fn, srcPage, dstPage)\n}\n\nfunc runCb298(fn func(src, output []byte) error, src, output []byte) error { return fn(src, output) }\n\nfunc honestProbe298(x *ImmutableReader, src, dst uint32) error {\n\tvar st tree.Store = honestStore298{r: x}\n\treturn st.CopyPage(src, dst, func(a, b []byte) error {\n\t\tcopy(b, a)\n\t\treturn nil\n\t})\n}\n"},
	}},

	{name: "P299: store callback wrapped in a local closure or identity alias forwarded to a helper with owned buffers fails closed", desc: "CopyPage(src, dst, fn) whose implementation hides the formal behind a local (cb := func(a, b []byte) error { return fn(a, b) } or cb := fn) and calls a scanned helper that forwards its byte parameters into the local: the store contract hands the callback mapped page views, so the closure/alias indirection must not launder owned buffers into the callback", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbwrap.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype liarWrap299 struct{ buf []byte }\n\nfunc (s liarWrap299) TargetTxn() uint64 { return 0 }\nfunc (s liarWrap299) PageLimit() uint64 { return 0 }\nfunc (s liarWrap299) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarWrap299) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarWrap299) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarWrap299) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarWrap299) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcb := func(a, b []byte) error { return fn(a, b) }\n\treturn runCb299(cb, s.buf, s.buf)\n}\n\nfunc runCb299(fn func(src, output []byte) error, src, output []byte) error { return fn(src, output) }\n\ntype liarAlias299 struct{ buf []byte }\n\nfunc (s liarAlias299) TargetTxn() uint64 { return 0 }\nfunc (s liarAlias299) PageLimit() uint64 { return 0 }\nfunc (s liarAlias299) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarAlias299) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarAlias299) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarAlias299) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarAlias299) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcb := fn\n\treturn runCb299(cb, s.buf, s.buf)\n}\n\nfunc liarWrapProbe299(page []byte) error {\n\tvar st tree.Store = liarWrap299{}\n\treturn st.CopyPage(0, 1, func(src, output []byte) error {\n\t\tcopy(output, page)\n\t\treturn nil\n\t})\n}\n"},
	}},

	{name: "P300 benign: store callback wrapped in a local closure or identity alias forwarded to a helper with mapped views passes", desc: "CopyPage(src, dst, fn) whose implementation sources mapped views from s.r.page and forwards them through a local wrapper (cb := func(a, b []byte) error { return fn(a, b) } or cb := fn) into a scanned helper: the store contract is satisfied through the indirection, and the honest wrapping must not be rejected", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbwrap_ok.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype honestWrap300 struct{ r *ImmutableReader }\n\nfunc (s honestWrap300) TargetTxn() uint64 { return 0 }\nfunc (s honestWrap300) PageLimit() uint64 { return 0 }\nfunc (s honestWrap300) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s honestWrap300) Allocate() (uint32, error) { return 0, nil }\nfunc (s honestWrap300) Update(uint32, func([]byte) error) error { return nil }\nfunc (s honestWrap300) DiscardPrivate(uint32) error { return nil }\n\nfunc (s honestWrap300) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tsrcPage, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\tdstPage, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\tcb := func(a, b []byte) error { return fn(a, b) }\n\treturn runCb300(cb, srcPage, dstPage)\n}\n\nfunc runCb300(fn func(src, output []byte) error, src, output []byte) error { return fn(src, output) }\n\ntype honestAlias300 struct{ r *ImmutableReader }\n\nfunc (s honestAlias300) TargetTxn() uint64 { return 0 }\nfunc (s honestAlias300) PageLimit() uint64 { return 0 }\nfunc (s honestAlias300) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s honestAlias300) Allocate() (uint32, error) { return 0, nil }\nfunc (s honestAlias300) Update(uint32, func([]byte) error) error { return nil }\nfunc (s honestAlias300) DiscardPrivate(uint32) error { return nil }\n\nfunc (s honestAlias300) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcb := fn\n\tsrcPage, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\tdstPage, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn runCb300(cb, srcPage, dstPage)\n}\n\nfunc honestWrapProbe300(x *ImmutableReader, src, dst uint32) error {\n\tvar st tree.Store = honestWrap300{r: x}\n\treturn st.CopyPage(src, dst, func(a, b []byte) error {\n\t\tcopy(b, a)\n\t\treturn nil\n\t})\n}\n"},
	}},

	{name: "P301: store callback invoked directly through an alias, a literal wrapper, or a struct field with owned buffers fails closed", desc: "CopyPage(src, dst, fn) implementations invoking the formal through a local identity alias (cb := fn; cb(s.buf, s.buf)), a declared-parameter literal wrapper (wrap := func(cb, a, b []byte) error { return cb(a, b) }; wrap(fn, s.buf, s.buf)), or a struct field (s.cb = fn; s.cb(s.buf, s.buf)): the invoke path is an unproven callee, so the owned full-page arguments must fail the fail-closed transfer exactly like the direct formal invocation", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbdir.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype liarDirAlias301 struct{ buf []byte }\n\nfunc (s liarDirAlias301) TargetTxn() uint64 { return 0 }\nfunc (s liarDirAlias301) PageLimit() uint64 { return 0 }\nfunc (s liarDirAlias301) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarDirAlias301) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarDirAlias301) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarDirAlias301) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarDirAlias301) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcb := fn\n\treturn cb(s.buf, s.buf)\n}\n\ntype liarDirWrap301 struct{ buf []byte }\n\nfunc (s liarDirWrap301) TargetTxn() uint64 { return 0 }\nfunc (s liarDirWrap301) PageLimit() uint64 { return 0 }\nfunc (s liarDirWrap301) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarDirWrap301) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarDirWrap301) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarDirWrap301) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarDirWrap301) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\twrap := func(cb func(src, output []byte) error, a, b []byte) error { return cb(a, b) }\n\treturn wrap(fn, s.buf, s.buf)\n}\n\ntype liarDirField301 struct {\n\tbuf []byte\n\tcb  func(src, output []byte) error\n}\n\nfunc (s liarDirField301) TargetTxn() uint64 { return 0 }\nfunc (s liarDirField301) PageLimit() uint64 { return 0 }\nfunc (s liarDirField301) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarDirField301) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarDirField301) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarDirField301) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarDirField301) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.cb = fn\n\treturn s.cb(s.buf, s.buf)\n}\n\nfunc liarDirProbe301(page []byte) error {\n\tvar st1 tree.Store = liarDirAlias301{}\n\tvar st2 tree.Store = liarDirWrap301{}\n\tvar st3 tree.Store = liarDirField301{}\n\t_ = st1.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n\t_ = st2.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n\treturn st3.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n}\n"},
	}},
	{name: "P302: store callback invoked through a dereferenced pointer with owned buffers fails closed", desc: "CopyPage(src, dst, fn) implementations invoking the formal through a pointer to a local alias (p := &cb; (*p)(s.buf, s.buf)): the (*p) caller expression would previously be misread as a TYPE CONVERSION (the syntactic isTypeExpr StarExpr case), skipping the unproven-callee page-argument fence entirely; the pointee is a variable, not a type, so the call must fail the full-page transfer like every other unproven callee indirection", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbptr.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype liarPtr302 struct{ buf []byte }\n\nfunc (s liarPtr302) TargetTxn() uint64 { return 0 }\nfunc (s liarPtr302) PageLimit() uint64 { return 0 }\nfunc (s liarPtr302) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarPtr302) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarPtr302) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarPtr302) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarPtr302) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcb := fn\n\tp := &cb\n\treturn (*p)(s.buf, s.buf)\n}\n\nfunc liarPtrProbe302(page []byte) error {\n\tvar st tree.Store = liarPtr302{}\n\treturn st.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n}\n"},
	}},
	{name: "P303: store callback invoked with a branch-joined capture (owned on one path) fails closed", desc: "CopyPage(src, dst, fn) whose helper captures a local (v := s.capture) into a wrapper closure, re-assigns v to a mapped mint on one branch only, and the helper invokes the callback formal with the capture: the value is not provably mapped on every path, so the join must fail closed instead of OR-ing the mapped mint over the owned path", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbjoin.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype liarJoin303 struct{ capture []byte; r *ImmutableReader }\n\nfunc (s liarJoin303) TargetTxn() uint64 { return 0 }\nfunc (s liarJoin303) PageLimit() uint64 { return 0 }\nfunc (s liarJoin303) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarJoin303) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarJoin303) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarJoin303) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarJoin303) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn joinMid303(fn, &s, x, y, source == 0)\n}\n\nfunc joinMid303(fn func(src, output []byte) error, s *liarJoin303, x, y []byte, fromPage bool) error {\n\tv := s.capture\n\tcb := func(pa, pb []byte) error { return fn(pa, v) }\n\tif fromPage {\n\t\tp, err := s.r.page(1)\n\t\tif err != nil {\n\t\t\treturn err\n\t\t}\n\t\tv = p\n\t}\n\treturn runCb303(cb, x, y)\n}\n\nfunc runCb303(fn func(a, b []byte) error, a, b []byte) error { return fn(a, b) }\n\nfunc liarJoinProbe303(page []byte, x *ImmutableReader) error {\n\tvar st tree.Store = liarJoin303{capture: page, r: x}\n\treturn st.CopyPage(0, 1, func(src, output []byte) error { copy(output, src); return nil })\n}\n"},
	}},
	{name: "P304: pointer-dereferenced callback calls fail closed even with mapped views (accepted conservatism)", desc: "A store implementation invoking its callback through a dereferenced local pointer with MAPPED views (*p)(x, y) has a callee body the scan cannot resolve; like every other unproven indirection (indexed, map-lookup, interface-dispatch), the full-page arguments fail closed. Production code must hand the callback to a scanned helper or call the formal directly", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbptr_ok.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype honestPtr304 struct{ r *ImmutableReader }\n\nfunc (s honestPtr304) TargetTxn() uint64 { return 0 }\nfunc (s honestPtr304) PageLimit() uint64 { return 0 }\nfunc (s honestPtr304) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s honestPtr304) Allocate() (uint32, error) { return 0, nil }\nfunc (s honestPtr304) Update(uint32, func([]byte) error) error { return nil }\nfunc (s honestPtr304) DiscardPrivate(uint32) error { return nil }\n\nfunc (s honestPtr304) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcb := fn\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\tp := &cb\n\treturn (*p)(x, y)\n}\n\nfunc honestPtrProbe304(x *ImmutableReader, page []byte) error {\n\tvar st tree.Store = honestPtr304{r: x}\n\treturn st.CopyPage(0, 1, func(src, output []byte) error { copy(output, src); return nil })\n}\n"},
	}},
	{name: "P305: block-scoped and var-declared callback aliases forwarded through a helper with owned buffers fail closed", desc: "CopyPage(src, dst, fn) implementations hiding the formal behind a block-scoped identity alias (if-block or switch-case cb := fn), a var-declared identity alias (var cb = fn, top-level or nested), or a block-scoped literal wrapper, and handing it to a scanned helper that forwards its byte parameters into the callback: noteCallbackAliases previously recorded aliases only from the top-level statement list, so the store-callback fence at the helper call site could not follow the block-scoped or var-declared bindings and the owned full-page arguments laundered into the callback", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbnest.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype liarNestIf305 struct{ buf []byte }\n\nfunc (s liarNestIf305) TargetTxn() uint64 { return 0 }\nfunc (s liarNestIf305) PageLimit() uint64 { return 0 }\nfunc (s liarNestIf305) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarNestIf305) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarNestIf305) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarNestIf305) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarNestIf305) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tif s.buf != nil {\n\t\tcb := fn\n\t\treturn runNest305(cb, s.buf, s.buf)\n\t}\n\treturn nil\n}\n\ntype liarNestSwitch305 struct{ buf []byte }\n\nfunc (s liarNestSwitch305) TargetTxn() uint64 { return 0 }\nfunc (s liarNestSwitch305) PageLimit() uint64 { return 0 }\nfunc (s liarNestSwitch305) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarNestSwitch305) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarNestSwitch305) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarNestSwitch305) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarNestSwitch305) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tswitch s.buf != nil {\n\tcase true:\n\t\tcb := fn\n\t\treturn runNest305(cb, s.buf, s.buf)\n\t}\n\treturn nil\n}\n\ntype liarNestVar305 struct{ buf []byte }\n\nfunc (s liarNestVar305) TargetTxn() uint64 { return 0 }\nfunc (s liarNestVar305) PageLimit() uint64 { return 0 }\nfunc (s liarNestVar305) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarNestVar305) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarNestVar305) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarNestVar305) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarNestVar305) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tvar cb = fn\n\treturn runNest305(cb, s.buf, s.buf)\n}\n\ntype liarNestWrap305 struct{ buf []byte }\n\nfunc (s liarNestWrap305) TargetTxn() uint64 { return 0 }\nfunc (s liarNestWrap305) PageLimit() uint64 { return 0 }\nfunc (s liarNestWrap305) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarNestWrap305) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarNestWrap305) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarNestWrap305) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarNestWrap305) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tif s.buf != nil {\n\t\tcb := func(a, b []byte) error { return fn(a, b) }\n\t\treturn runNest305(cb, s.buf, s.buf)\n\t}\n\treturn nil\n}\n\nfunc runNest305(fn func(src, output []byte) error, src, output []byte) error { return fn(src, output) }\n\nfunc liarNestProbe305(page []byte) error {\n\tvar st1 tree.Store = liarNestIf305{}\n\tvar st2 tree.Store = liarNestSwitch305{}\n\tvar st3 tree.Store = liarNestVar305{}\n\tvar st4 tree.Store = liarNestWrap305{}\n\t_ = st1.CopyPage(0, 1, func(src, output []byte) error { copy(output, src); return nil })\n\t_ = st2.CopyPage(0, 1, func(src, output []byte) error { copy(output, src); return nil })\n\t_ = st3.CopyPage(0, 1, func(src, output []byte) error { copy(output, src); return nil })\n\treturn st4.CopyPage(0, 1, func(src, output []byte) error { copy(output, src); return nil })\n}\n"},
	}},
	{name: "P306 benign: block-scoped and var-declared callback aliases forwarded through a helper with mapped views pass", desc: "CopyPage(src, dst, fn) implementations sourcing mapped views from s.r.page and forwarding them through block-scoped or var-declared identity aliases into a scanned helper: the store contract is satisfied through the indirection, and the nested/var declarations must not be over-rejected", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbnest_ok.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype honestNestIf306 struct{ r *ImmutableReader }\n\nfunc (s honestNestIf306) TargetTxn() uint64 { return 0 }\nfunc (s honestNestIf306) PageLimit() uint64 { return 0 }\nfunc (s honestNestIf306) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s honestNestIf306) Allocate() (uint32, error) { return 0, nil }\nfunc (s honestNestIf306) Update(uint32, func([]byte) error) error { return nil }\nfunc (s honestNestIf306) DiscardPrivate(uint32) error { return nil }\n\nfunc (s honestNestIf306) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\tif x != nil {\n\t\tcb := fn\n\t\treturn runNest306(cb, x, y)\n\t}\n\treturn nil\n}\n\ntype honestNestVar306 struct{ r *ImmutableReader }\n\nfunc (s honestNestVar306) TargetTxn() uint64 { return 0 }\nfunc (s honestNestVar306) PageLimit() uint64 { return 0 }\nfunc (s honestNestVar306) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s honestNestVar306) Allocate() (uint32, error) { return 0, nil }\nfunc (s honestNestVar306) Update(uint32, func([]byte) error) error { return nil }\nfunc (s honestNestVar306) DiscardPrivate(uint32) error { return nil }\n\nfunc (s honestNestVar306) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\tvar cb = fn\n\treturn runNest306(cb, x, y)\n}\n\nfunc runNest306(fn func(src, output []byte) error, src, output []byte) error { return fn(src, output) }\n\nfunc honestNestProbe306(x *ImmutableReader, src, dst uint32) error {\n\tvar st1 tree.Store = honestNestIf306{r: x}\n\tvar st2 tree.Store = honestNestVar306{r: x}\n\t_ = st1.CopyPage(src, dst, func(a, b []byte) error { copy(b, a); return nil })\n\treturn st2.CopyPage(src, dst, func(a, b []byte) error { copy(b, a); return nil })\n}\n"},
	}},
	{name: "P307: store callback invoked through field, alias, wrapper, or helper-field indirection with clean owned buffers fails closed", desc: "CopyPage(src, dst, fn) implementations handing the callback to an owned make([]byte, 4096) buffer through a struct field (s.cb = fn; s.cb(out, out)), a local identity alias (cb := fn; cb(out, out)), a forwarder literal (wrap := func(cb, a, b []byte) error { return cb(a, b) }; wrap(fn, out, out)), or a helper that stores the formal in a local struct field and invokes it there: the buffers carry no page taint, so the generic unproven-callee fence cannot see the transfer; the store-callback counter-check must require mapped views at every invocation point reachable from the store implementation. P301/P243 pinned the tainted-buffer forms; these pins prove the clean-buffer forms fail too", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbclean.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype liarCleanField307 struct{ cb func(src, output []byte) error }\n\nfunc (s liarCleanField307) TargetTxn() uint64 { return 0 }\nfunc (s liarCleanField307) PageLimit() uint64 { return 0 }\nfunc (s liarCleanField307) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarCleanField307) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarCleanField307) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarCleanField307) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarCleanField307) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.cb = fn\n\tout := make([]byte, 4096)\n\treturn s.cb(out, out)\n}\n\ntype liarCleanAlias307 struct{}\n\nfunc (s liarCleanAlias307) TargetTxn() uint64 { return 0 }\nfunc (s liarCleanAlias307) PageLimit() uint64 { return 0 }\nfunc (s liarCleanAlias307) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarCleanAlias307) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarCleanAlias307) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarCleanAlias307) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarCleanAlias307) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcb := fn\n\tout := make([]byte, 4096)\n\treturn cb(out, out)\n}\n\ntype liarCleanWrap307 struct{}\n\nfunc (s liarCleanWrap307) TargetTxn() uint64 { return 0 }\nfunc (s liarCleanWrap307) PageLimit() uint64 { return 0 }\nfunc (s liarCleanWrap307) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarCleanWrap307) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarCleanWrap307) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarCleanWrap307) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarCleanWrap307) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\twrap := func(cb func(src, output []byte) error, a, b []byte) error { return cb(a, b) }\n\tout := make([]byte, 4096)\n\treturn wrap(fn, out, out)\n}\n\ntype liarCleanHelper307 struct{}\n\nfunc (s liarCleanHelper307) TargetTxn() uint64 { return 0 }\nfunc (s liarCleanHelper307) PageLimit() uint64 { return 0 }\nfunc (s liarCleanHelper307) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarCleanHelper307) Allocate() (uint32, error) { return 0, nil }\nfunc (s liarCleanHelper307) Update(uint32, func([]byte) error) error { return nil }\nfunc (s liarCleanHelper307) DiscardPrivate(uint32) error { return nil }\n\nfunc (s liarCleanHelper307) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\treturn runCbClean307(fn)\n}\n\nfunc runCbClean307(fn func(src, output []byte) error) error {\n\tvar h struct{ f func(src, output []byte) error }\n\th.f = fn\n\tout := make([]byte, 4096)\n\treturn h.f(out, out)\n}\n\nfunc liarCleanProbe307(page []byte) error {\n\tvar st1 tree.Store = liarCleanField307{}\n\tvar st2 tree.Store = liarCleanAlias307{}\n\tvar st3 tree.Store = liarCleanWrap307{}\n\tvar st4 tree.Store = liarCleanHelper307{}\n\t_ = st1.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n\t_ = st2.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n\t_ = st3.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n\treturn st4.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n}\n"},
	}},
	{name: "P308 benign: block-scoped literal wrapper forwarded through a helper with mapped views passes", desc: "CopyPage(src, dst, fn) implementations sourcing mapped views from s.r.page and forwarding them through a block-scoped literal wrapper (if x != nil { cb := func(a, b []byte) error { return fn(a, b) }; return runNest308(cb, x, y) }): the store contract is satisfied through the indirection and the nested wrapper declaration must not be over-rejected by the callback-alias or counter-check rules", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_cbnest_ok2.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype honestNestWrap308 struct{ r *ImmutableReader }\n\nfunc (s honestNestWrap308) TargetTxn() uint64 { return 0 }\nfunc (s honestNestWrap308) PageLimit() uint64 { return 0 }\nfunc (s honestNestWrap308) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s honestNestWrap308) Allocate() (uint32, error) { return 0, nil }\nfunc (s honestNestWrap308) Update(uint32, func([]byte) error) error { return nil }\nfunc (s honestNestWrap308) DiscardPrivate(uint32) error { return nil }\n\nfunc (s honestNestWrap308) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\tif x != nil {\n\t\tcb := func(a, b []byte) error { return fn(a, b) }\n\t\treturn runNest308(cb, x, y)\n\t}\n\treturn nil\n}\n\nfunc runNest308(fn func(src, output []byte) error, src, output []byte) error { return fn(src, output) }\n\nfunc honestNestWrapProbe308(x *ImmutableReader, src, dst uint32) error {\n\tvar st tree.Store = honestNestWrap308{r: x}\n\treturn st.CopyPage(src, dst, func(a, b []byte) error { copy(b, a); return nil })\n}\n"},
	}},
	{name: "P309: helper-local trailing mint and cross-helper alias chain launder the store callback", desc: "CopyPage(src, dst, fn) implementations that (a) hand the callback formal to a helper which invokes it with a LOCAL owned buffer and then re-mints the local from the mapping AFTER the invocation, or (b) alias the formal in one helper (cb := fn) and forward the alias to a second helper that invokes it with owned buffers: the definition-site mapped-exemption must not read the trailing mint's end-state value, and the composition must resolve the alias recorded in the first helper, or the owned full-page views launder into the callback", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_lin.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\n// P309a: helper-local owned buffer laundered into the callback formal\n// with a trailing mint AFTER the invocation (P2-1 end-state divergence).\ntype liarLin309 struct{ buf []byte; r *ImmutableReader }\n\nfunc (s liarLin309) TargetTxn() uint64                        { return 0 }\nfunc (s liarLin309) PageLimit() uint64                        { return 0 }\nfunc (s liarLin309) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarLin309) Allocate() (uint32, error)                { return 0, nil }\nfunc (s liarLin309) Update(uint32, func([]byte) error) error  { return nil }\nfunc (s liarLin309) DiscardPrivate(uint32) error              { return nil }\n\nfunc (s liarLin309) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\treturn launderLocalLin309(fn, &s)\n}\n\nfunc launderLocalLin309(fn func(src, output []byte) error, s *liarLin309) error {\n\tv := s.buf\n\terr := fn(v, v)\n\tp, perr := s.r.page(0)\n\tif perr != nil {\n\t\treturn perr\n\t}\n\tv = p\n\t_ = v\n\treturn err\n}\n\n// P309b: cross-helper alias chain (P2-2): the formal is aliased in one\n// helper and the alias is forwarded to a second helper whose summary\n// carries the traced invocation.\ntype liarChainLin309 struct{ buf []byte }\n\nfunc (s liarChainLin309) TargetTxn() uint64                        { return 0 }\nfunc (s liarChainLin309) PageLimit() uint64                        { return 0 }\nfunc (s liarChainLin309) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s liarChainLin309) Allocate() (uint32, error)                { return 0, nil }\nfunc (s liarChainLin309) Update(uint32, func([]byte) error) error  { return nil }\nfunc (s liarChainLin309) DiscardPrivate(uint32) error              { return nil }\n\nfunc (s liarChainLin309) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\treturn h1Lin309(fn, &s)\n}\n\nfunc h1Lin309(fn func(src, output []byte) error, s *liarChainLin309) error {\n\tcb := fn\n\treturn h2Lin309(cb, s.buf, s.buf)\n}\n\nfunc h2Lin309(fn func(src, output []byte) error, a, b []byte) error { return fn(a, b) }\n\nfunc liarProbe309(page []byte, x *ImmutableReader) error {\n\tvar st1 tree.Store = liarLin309{buf: page, r: x}\n\tvar st2 tree.Store = liarChainLin309{buf: page}\n\t_ = st1.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n\treturn st2.CopyPage(0, 1, func(src, output []byte) error { copy(output, page); return nil })\n}"},
	}},
...
`},
	...
	{name: "P317: any()-converted callback field asserted and invoked with owned buffers launder the store callback", desc: "CopyPage(src, dst, fn) implementations storing the callback formal into an interface field and invoking it through an any()-conversion assert-to-local (f := any(s.cb).(cbSig); f(out, out)): type erasure hides the callback alias from the direct field path, so the conversion must resolve the asserted value to the stored formal and enforce the mapped-view rule at the call", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m317.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\n// M317: assert-to-local on an any()-converted callback field (K2 class).\ntype m317 struct{ cb any }\n\nfunc (s *m317) TargetTxn() uint64                        { return 0 }\nfunc (s *m317) PageLimit() uint64                        { return 0 }\nfunc (s *m317) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m317) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m317) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m317) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m317) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.cb = fn\n\tout := make([]byte, 4096)\n\tf := any(s.cb).(func(src, output []byte) error)\n\treturn f(out, out)\n}\n\n\n// M317b: any()-converted callback field asserted and forwarded to a\n// scanned helper with owned buffers (helper-forward twin of the K2\n// class: the helper fence must accept the recorded alias, then the\n// byte-argument counter-check must still require mapped views).\ntype m317b struct{ cb any }\n\nfunc (s *m317b) TargetTxn() uint64                        { return 0 }\nfunc (s *m317b) PageLimit() uint64                        { return 0 }\nfunc (s *m317b) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m317b) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m317b) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m317b) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m317b) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.cb = fn\n\tout := make([]byte, 4096)\n\tf := any(s.cb).(func(src, output []byte) error)\n\treturn runJ317M(f, out, out)\n}\n\nfunc runJ317M(f func(src, output []byte) error, a, b []byte) error { return f(a, b) }\n\nfunc liarProbeM317() error {\n\tvar a tree.Store = &m317{}\n\tvar b tree.Store = &m317b{}\n\t_ = a.CopyPage(0, 1, func(src, output []byte) error { return nil })\n\treturn b.CopyPage(0, 1, func(src, output []byte) error { return nil })\n}\n"},
	}},
	{name: "P318: call-result getters returning the stored callback launder the store callback", desc: "CopyPage(src, dst, fn) implementations invoking the stored callback through a method getter (s.getCB()(out, out)) or a free-function getter (freeGet(s)(out, out)): the returned func-typed value is the callback formal, so the callee-side return summary must resolve result position to the canonical field or parameter alias and require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m318.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\n// M318a: method-returned formal callee (K3 class).\ntype m318a struct{ cb func(src, output []byte) error }\n\nfunc (s *m318a) getCB() func(src, output []byte) error { return s.cb }\n\nfunc (s *m318a) TargetTxn() uint64                        { return 0 }\nfunc (s *m318a) PageLimit() uint64                        { return 0 }\nfunc (s *m318a) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m318a) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m318a) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m318a) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m318a) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.cb = fn\n\tout := make([]byte, 4096)\n\treturn s.getCB()(out, out)\n}\n\n// M318b: free-function returned formal callee (E1b class).\ntype m318b struct{ cb func(src, output []byte) error }\n\nfunc freeGetM318(s *m318b) func(src, output []byte) error { return s.cb }\n\nfunc (s *m318b) TargetTxn() uint64                        { return 0 }\nfunc (s *m318b) PageLimit() uint64                        { return 0 }\nfunc (s *m318b) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m318b) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m318b) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m318b) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m318b) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.cb = fn\n\tout := make([]byte, 4096)\n\treturn freeGetM318(s)(out, out)\n}\n\nfunc liarProbeM318() error {\n\tvar a tree.Store = &m318a{}\n\tvar b tree.Store = &m318b{}\n\t_ = a.CopyPage(0, 1, func(src, output []byte) error { return nil })\n\treturn b.CopyPage(0, 1, func(src, output []byte) error { return nil })\n}\n"},
	}},
	{name: "P319: carriers born inside argument expressions launder the store callback", desc: "CopyPage(src, dst, fn) implementations reaching the callback through a carrier struct created inside the call argument: a composite literal (runCar(car{cb: fn}, out, out)), a helper-returned carrier (runCar(mkCar(fn), out, out)), or a two-helper chain (runOuter(mkCar(fn), out, out)): the carrier never appears in a named local, so the return-carrier composition must seed the helper's field aliases from the literal and the helper fence must require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m319.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM319 struct{ cb func(src, output []byte) error }\n\nfunc runCarM319a(c carM319, a, b []byte) error { return c.cb(a, b) }\nfunc runCarM319b(c carM319, a, b []byte) error { return c.cb(a, b) }\nfunc mkCarM319(fn func(src, output []byte) error) carM319 { return carM319{cb: fn} }\nfunc runOuterM319(c carM319, a, b []byte) error { return runInnerM319(c, a, b) }\nfunc runInnerM319(c carM319, a, b []byte) error { return c.cb(a, b) }\n\n// M319a: composite-literal carrier passed directly to a helper (K4 class).\ntype m319a struct{}\n\nfunc (s *m319a) TargetTxn() uint64                        { return 0 }\nfunc (s *m319a) PageLimit() uint64                        { return 0 }\nfunc (s *m319a) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m319a) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m319a) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m319a) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m319a) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tout := make([]byte, 4096)\n\treturn runCarM319a(carM319{cb: fn}, out, out)\n}\n\n// M319b: composite-literal carrier passed to a second helper name (E2b class).\ntype m319b struct{}\n\nfunc (s *m319b) TargetTxn() uint64                        { return 0 }\nfunc (s *m319b) PageLimit() uint64                        { return 0 }\nfunc (s *m319b) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m319b) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m319b) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m319b) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m319b) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tout := make([]byte, 4096)\n\treturn runCarM319b(carM319{cb: fn}, out, out)\n}\n\n// M319c: helper-returned carrier (E2a class).\ntype m319c struct{}\n\nfunc (s *m319c) TargetTxn() uint64                        { return 0 }\nfunc (s *m319c) PageLimit() uint64                        { return 0 }\nfunc (s *m319c) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m319c) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m319c) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m319c) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m319c) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tout := make([]byte, 4096)\n\treturn runCarM319a(mkCarM319(fn), out, out)\n}\n\n// M319d: two-helper chain with the returned carrier (E2c class).\ntype m319d struct{}\n\nfunc (s *m319d) TargetTxn() uint64                        { return 0 }\nfunc (s *m319d) PageLimit() uint64                        { return 0 }\nfunc (s *m319d) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m319d) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m319d) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m319d) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m319d) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tout := make([]byte, 4096)\n\treturn runOuterM319(mkCarM319(fn), out, out)\n}\n\nfunc liarProbeM319() error {\n\tvar a tree.Store = &m319a{}\n\tvar b tree.Store = &m319b{}\n\tvar c tree.Store = &m319c{}\n\tvar d tree.Store = &m319d{}\n\t_ = a.CopyPage(0, 1, func(src, output []byte) error { return nil })\n\t_ = b.CopyPage(0, 1, func(src, output []byte) error { return nil })\n\t_ = c.CopyPage(0, 1, func(src, output []byte) error { return nil })\n\treturn d.CopyPage(0, 1, func(src, output []byte) error { return nil })\n}\n"},
	}},
	{name: "P320 benign: argument-carrier and any()-conversion forms with mapped views pass", desc: "CopyPage(src, dst, fn) implementations sourcing mapped views from r.page and forwarding them through a helper-returned carrier (runCar(mkCar(fn), x, y)) or through an any()-asserted callback handed to a scanned helper (f := any(s.cb).(cbSig); runJ320M(f, x, y)): the helper fence resolves the callback alias and sees mapped byte arguments, so it must not over-reject the honest twins of P317/P319", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m320.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM320 struct{ cb func(src, output []byte) error }\n\nfunc runCarM320(c carM320, a, b []byte) error { return c.cb(a, b) }\nfunc mkCarM320(fn func(src, output []byte) error) carM320 { return carM320{cb: fn} }\n\nfunc runJ320M(f func(src, output []byte) error, a, b []byte) error { return f(a, b) }\n\n// M320a: helper-returned carrier with mapped views (E2d twin).\ntype m320a struct{ r *ImmutableReader }\n\nfunc (s *m320a) TargetTxn() uint64                        { return 0 }\nfunc (s *m320a) PageLimit() uint64                        { return 0 }\nfunc (s *m320a) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m320a) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m320a) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m320a) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m320a) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn runCarM320(mkCarM320(fn), x, y)\n}\n\n// M320b: any()-converted callback field asserted and forwarded to a\n// scanned helper with mapped views (honest twin of the M317 liar).\ntype m320b struct {\n\tcb any\n\tr  *ImmutableReader\n}\n\nfunc (s *m320b) TargetTxn() uint64                        { return 0 }\nfunc (s *m320b) PageLimit() uint64                        { return 0 }\nfunc (s *m320b) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m320b) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m320b) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m320b) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m320b) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.cb = fn\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\tf := any(s.cb).(func(src, output []byte) error)\n\treturn runJ320M(f, x, y)\n}\n\nfunc honestProbeM320(x *ImmutableReader, src, dst uint32) error {\n\tvar a tree.Store = &m320a{r: x}\n\tvar b tree.Store = &m320b{r: x}\n\t_ = a.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn b.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n}\n"},
	}},
	{name: "P321: passthrough identity returns launder the store callback", desc: "CopyPage(src, dst, fn) implementations running cb := passthrough(fn) then cb(out, out) with owned buffers: the helper return resolves to the callback formal parameter, so the invocation must enforce the mapped-view counter-check at the call", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m321.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\nfunc passthroughM321(f func(src, output []byte) error) func(src, output []byte) error { return f }\n\ntype m321 struct{}\n\nfunc (s *m321) TargetTxn() uint64                        { return 0 }\nfunc (s *m321) PageLimit() uint64                        { return 0 }\nfunc (s *m321) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m321) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m321) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m321) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m321) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcb := passthroughM321(fn)\n\tout := make([]byte, 4096)\n\treturn cb(out, out)\n}\n\nfunc liarProbeM321() error {\n\tvar a tree.Store = &m321{}\n\treturn a.CopyPage(0, 1, func(src, output []byte) error { return nil })\n}\n"},
	}},
	{name: "P322: positional struct composites launder the store callback", desc: "CopyPage(src, dst, fn) implementations with h := carM322{fn} (positional composite of a named carrier type) then h.cb(out, out): the positional element must seed the canonical field alias through the named underlying struct, and the byte-argument check must require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m322.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM322 struct{ cb func(src, output []byte) error }\n\ntype m322 struct{}\n\nfunc (s *m322) TargetTxn() uint64                        { return 0 }\nfunc (s *m322) PageLimit() uint64                        { return 0 }\nfunc (s *m322) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m322) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m322) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m322) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m322) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\th := carM322{fn}\n\tout := make([]byte, 4096)\n\treturn h.cb(out, out)\n}\n\nfunc liarProbeM322() error {\n\tvar a tree.Store = &m322{}\n\treturn a.CopyPage(0, 1, func(src, output []byte) error { return nil })\n}\n"},
	}},
	{name: "P323: cross-method field storage launders the store callback", desc: "CopyPage(src, dst, fn) implementations calling s.prep(fn) (a method storing the formal in s.cb) then invoking s.cb with helper-produced owned buffers: the cross-method field binding must compose the setter's direct field record into the store summary and the invocation must require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m323.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype m323 struct{ cb func(src, output []byte) error }\n\nfunc (s *m323) prep(fn func(src, output []byte) error) { s.cb = fn }\n\nfunc (s *m323) TargetTxn() uint64                        { return 0 }\nfunc (s *m323) PageLimit() uint64                        { return 0 }\nfunc (s *m323) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m323) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m323) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m323) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m323) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.prep(fn)\n\treturn s.cb(outM323(), outM323())\n}\n\nfunc outM323() []byte { return make([]byte, 4096) }\n\nfunc liarProbeM323() error {\n\tvar a tree.Store = &m323{}\n\treturn a.CopyPage(0, 1, func(src, output []byte) error { return nil })\n}\n"},
	}},
	{name: "P324: wrapper literals carrying pair composites launder the store callback", desc: "CopyPage(src, dst, fn) implementations invoking a local wrapper literal with the formal and a pair composite of OWNED buffers (wrap(fn, pair{a: buf, b: buf})): the wrapper-body callback invocation composes into the store summary and the pair composite's byte fields must be provably mapped, so owned elements fail the counter-check", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m324.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype pairM324 struct{ a, b []byte }\n\ntype m324 struct{ buf []byte }\n\nfunc (s *m324) TargetTxn() uint64                        { return 0 }\nfunc (s *m324) PageLimit() uint64                        { return 0 }\nfunc (s *m324) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m324) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m324) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m324) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m324) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\twrap := func(cb func(src, output []byte) error, v pairM324) error { return cb(v.a, v.b) }\n\treturn wrap(fn, pairM324{a: s.buf, b: s.buf})\n}\n\nfunc liarProbeM324() error {\n\tvar a tree.Store = &m324{buf: make([]byte, 4096)}\n\treturn a.CopyPage(0, 1, func(src, output []byte) error { return nil })\n}\n"},
	}},
	{name: "P325: setter-bound carriers launder the store callback", desc: "CopyPage(src, dst, fn) implementations binding the formal through a package-global setter (setGlobal(fn); g.cb(out, out)) or a pointer setter (var h carrier; setCb(&h, fn); h.cb(out, out)): the setter helpers' direct field records must compose into the store implementation summary and the invocation must require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m325.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM325 struct{ cb func(src, output []byte) error }\n\nvar gM325 carM325\n\nfunc setGlobalM325(fn func(src, output []byte) error) { gM325.cb = fn }\nfunc setCbM325(h *carM325, fn func(src, output []byte) error) { h.cb = fn }\n\ntype m325a struct{}\n\ntype m325b struct{}\n\nfunc (s *m325a) TargetTxn() uint64                        { return 0 }\nfunc (s *m325a) PageLimit() uint64                        { return 0 }\nfunc (s *m325a) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m325a) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m325a) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m325a) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m325a) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tsetGlobalM325(fn)\n\tout := make([]byte, 4096)\n\treturn gM325.cb(out, out)\n}\n\nfunc (s *m325b) TargetTxn() uint64                        { return 0 }\nfunc (s *m325b) PageLimit() uint64                        { return 0 }\nfunc (s *m325b) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m325b) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m325b) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m325b) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m325b) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tvar h carM325\n\tsetCbM325(&h, fn)\n\tout := make([]byte, 4096)\n\treturn h.cb(out, out)\n}\n\nfunc liarProbeM325() error {\n\tvar a tree.Store = &m325a{}\n\tvar b tree.Store = &m325b{}\n\t_ = a.CopyPage(0, 1, func(src, output []byte) error { return nil })\n\treturn b.CopyPage(0, 1, func(src, output []byte) error { return nil })\n}\n"},
	}},
	{name: "P326 benign: helper-forwarding honest twins of the round-2 callback classes pass", desc: "CopyPage(src, dst, fn) implementations sourcing MAPPED views from r.page and invoking the callback only through scanned helpers - global setter (runGlobal(x, y)), pointer setter (runLocal(&h, x, y)), passthrough (runPassthrough(cb, x, y)), positional composite (runPos(car{fn}, x, y)), cross-method (runCross(s, x, y)), wrapper pair composite (runWrap(fn, pair{a: x, b: y})), and bound-carrier launch (launch(car{cb: fn}, x)): the direct field-call forms stay fail-closed (P304 class), so every honest twin must forward through helpers; none may be over-rejected", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m326.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype hcarM326 struct{ cb func(src, output []byte) error }\n\nvar hGlobalM326 hcarM326\n\ntype pairM326 struct{ a, b []byte }\n\nfunc setGlobalM326(fn func(src, output []byte) error) { hGlobalM326.cb = fn }\nfunc setCbM326(h *hcarM326, fn func(src, output []byte) error) { h.cb = fn }\nfunc passthroughM326(f func(src, output []byte) error) func(src, output []byte) error { return f }\n\nfunc runGlobalM326(a, b []byte) error           { return hGlobalM326.cb(a, b) }\nfunc runLocalM326(h *hcarM326, a, b []byte) error { return h.cb(a, b) }\nfunc runPastM326(f func(src, output []byte) error, a, b []byte) error { return f(a, b) }\nfunc runPosM326(h hcarM326, a, b []byte) error   { return h.cb(a, b) }\nfunc runWrapM326(cb func(src, output []byte) error, v pairM326) error { return cb(v.a, v.b) }\n\ntype carS2cM326 struct{ cb func(a, b []byte) error }\n\nfunc hS2cM326(c carS2cM326, page []byte) error       { return c.cb(page, page) }\nfunc launchS2cM326(car carS2cM326, page []byte) error { return hS2cM326(car, page) }\n\ntype m326s struct{ r *ImmutableReader }\n\ntype m326a struct{ r *ImmutableReader }\ntype m326b struct{ r *ImmutableReader }\ntype m326d struct{ r *ImmutableReader }\ntype m326e struct{ r *ImmutableReader }\ntype m326f struct{ r *ImmutableReader }\ntype m326c struct{ r *ImmutableReader }\n\ntype m326g struct {\n\tr   *ImmutableReader\n\tcb  func(src, output []byte) error\n\tbuf []byte\n}\n\nfunc (s *m326g) prep(fn func(src, output []byte) error) { s.cb = fn }\nfunc runCrossM326(s *m326g, a, b []byte) error            { return s.cb(a, b) }\n\nfunc (s *m326a) TargetTxn() uint64                        { return 0 }\nfunc (s *m326a) PageLimit() uint64                        { return 0 }\nfunc (s *m326a) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m326a) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m326a) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m326a) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m326a) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tsetGlobalM326(fn)\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn runGlobalM326(x, y)\n}\n\nfunc (s *m326b) TargetTxn() uint64                        { return 0 }\nfunc (s *m326b) PageLimit() uint64                        { return 0 }\nfunc (s *m326b) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m326b) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m326b) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m326b) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m326b) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tvar h hcarM326\n\tsetCbM326(&h, fn)\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn runLocalM326(&h, x, y)\n}\n\nfunc (s *m326d) TargetTxn() uint64                        { return 0 }\nfunc (s *m326d) PageLimit() uint64                        { return 0 }\nfunc (s *m326d) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m326d) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m326d) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m326d) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m326d) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcb := passthroughM326(fn)\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn runPastM326(cb, x, y)\n}\n\nfunc (s *m326e) TargetTxn() uint64                        { return 0 }\nfunc (s *m326e) PageLimit() uint64                        { return 0 }\nfunc (s *m326e) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m326e) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m326e) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m326e) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m326e) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\th := hcarM326{fn}\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn runPosM326(h, x, y)\n}\n\nfunc (s *m326g) TargetTxn() uint64                        { return 0 }\nfunc (s *m326g) PageLimit() uint64                        { return 0 }\nfunc (s *m326g) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m326g) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m326g) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m326g) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m326g) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.prep(fn)\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn runCrossM326(s, x, y)\n}\n\nfunc (s *m326f) TargetTxn() uint64                        { return 0 }\nfunc (s *m326f) PageLimit() uint64                        { return 0 }\nfunc (s *m326f) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m326f) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m326f) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m326f) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m326f) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn runWrapM326(fn, pairM326{a: x, b: y})\n}\n\nfunc (s *m326c) TargetTxn() uint64                        { return 0 }\nfunc (s *m326c) PageLimit() uint64                        { return 0 }\nfunc (s *m326c) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m326c) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m326c) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m326c) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m326c) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tcar := carS2cM326{cb: fn}\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn launchS2cM326(car, x)\n}\n\nfunc honestProbeM326(x *ImmutableReader, src, dst uint32) error {\n\tvar a tree.Store = &m326a{r: x}\n\tvar b tree.Store = &m326b{r: x}\n\tvar d tree.Store = &m326d{r: x}\n\tvar e tree.Store = &m326e{r: x}\n\tvar g tree.Store = &m326g{r: x}\n\tvar f tree.Store = &m326f{r: x}\n\tvar c tree.Store = &m326c{r: x}\n\t_ = a.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\t_ = b.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\t_ = d.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\t_ = e.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\t_ = g.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\t_ = f.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn c.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n}\n"},
	}},
	{name: "P327: local func-var passthrough launder", desc: "CopyPage(src, dst, fn) implementations binding a package function to a LOCAL func-typed variable (g := passthrough; cb := g(fn)) then invoking cb(out, out) with owned buffers: the local-var callee resolution must trace g to the scanned passthrough summary and enforce the mapped-view counter-check at the invocation", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m327.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\nfunc passthroughM327(f func(src, output []byte) error) func(src, output []byte) error { return f }\n\ntype m327 struct{}\n\nfunc (s *m327) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tg := passthroughM327\n\tcb := g(fn)\n\tout := make([]byte, 4096)\n\treturn cb(out, out)\n}\nfunc (s *m327) TargetTxn() uint64                        { return 0 }\nfunc (s *m327) PageLimit() uint64                        { return 0 }\nfunc (s *m327) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m327) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m327) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m327) Update(uint32, func([]byte) error) error  { return nil }\n\n\nfunc liarProbeM327() error {\n\tvar s0 tree.Store = &m327{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P328: local-bound method-value getter launder", desc: "CopyPage(src, dst, fn) implementations with mv := s.getCB (a method VALUE bound to a local) then cb := mv() and cb(out, out): the local method-value callee must resolve through the getter summary and the invocation must require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m328.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype m328 struct{ cb func(src, output []byte) error }\n\nfunc (s *m328) getCB() func(src, output []byte) error { return s.cb }\n\nfunc (s *m328) TargetTxn() uint64                        { return 0 }\nfunc (s *m328) PageLimit() uint64                        { return 0 }\nfunc (s *m328) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m328) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m328) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m328) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m328) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\ts.cb = fn\n\tmv := s.getCB\n\tcb := mv()\n\tout := make([]byte, 4096)\n\treturn cb(out, out)\n}\n\nfunc liarProbeM328() error {\n\tvar s0 tree.Store = &m328{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P329: keyed nested carrier in call argument launder", desc: "CopyPage(src, dst, fn) implementations passing a KEYED nested carrier composite directly in the helper call argument (runOuter(outer{in: car{cb: fn}}, out, out)): the expression-position composite must seed the leaf field record with the outer path and the helper fence must require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m329.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM329 struct{ cb func(src, output []byte) error }\n\ntype outerM329 struct{ in carM329 }\n\nfunc runOuterM329(o outerM329, a, b []byte) error { return o.in.cb(a, b) }\n\ntype m329 struct{}\n\nfunc (s *m329) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tout := make([]byte, 4096)\n\treturn runOuterM329(outerM329{in: carM329{cb: fn}}, out, out)\n}\nfunc (s *m329) TargetTxn() uint64                        { return 0 }\nfunc (s *m329) PageLimit() uint64                        { return 0 }\nfunc (s *m329) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m329) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m329) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m329) Update(uint32, func([]byte) error) error  { return nil }\n\n\nfunc liarProbeM329() error {\n\tvar s0 tree.Store = &m329{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P330: three-level nested carrier in call argument launder", desc: "CopyPage(src, dst, fn) implementations passing a 3-level nested carrier composite in the call argument (runTop(top{mid{outer{in: car{cb: fn}}}}, out, out)): the leaf record must carry the full outer path and match the runTop parameter declared as the outermost type", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m330.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM330 struct{ cb func(src, output []byte) error }\n\ntype outerM330 struct{ in carM330 }\n\ntype midM330 struct{ o outerM330 }\n\ntype topM330 struct{ m midM330 }\n\nfunc runTopM330(t topM330, a, b []byte) error { return t.m.o.in.cb(a, b) }\n\ntype m330 struct{}\n\nfunc (s *m330) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tout := make([]byte, 4096)\n\treturn runTopM330(topM330{midM330{outerM330{in: carM330{cb: fn}}}}, out, out)\n}\nfunc (s *m330) TargetTxn() uint64                        { return 0 }\nfunc (s *m330) PageLimit() uint64                        { return 0 }\nfunc (s *m330) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m330) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m330) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m330) Update(uint32, func([]byte) error) error  { return nil }\n\n\nfunc liarProbeM330() error {\n\tvar s0 tree.Store = &m330{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P331: nested carrier via inner-field setter launder", desc: "CopyPage(src, dst, fn) implementations with var o outer; o.in = car{cb: fn} then runOuter(o, out, out): the selector-destination composite must seed the leaf record with the destination steps and the outer-carrier fence must require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m331.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM331 struct{ cb func(src, output []byte) error }\n\ntype outerM331 struct{ in carM331 }\n\nfunc runOuterM331(o outerM331, a, b []byte) error { return o.in.cb(a, b) }\n\ntype m331 struct{}\n\nfunc (s *m331) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tvar o outerM331\n\to.in = carM331{cb: fn}\n\tout := make([]byte, 4096)\n\treturn runOuterM331(o, out, out)\n}\nfunc (s *m331) TargetTxn() uint64                        { return 0 }\nfunc (s *m331) PageLimit() uint64                        { return 0 }\nfunc (s *m331) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m331) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m331) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m331) Update(uint32, func([]byte) error) error  { return nil }\n\n\nfunc liarProbeM331() error {\n\tvar s0 tree.Store = &m331{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P332: local-bound nested carrier launder", desc: "CopyPage(src, dst, fn) implementations with o := outer{car{fn}} then runOuter(o, out, out): the positional nested composite bound to a local must seed the leaf record with the outer path and the fence must require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m332.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM332 struct{ cb func(src, output []byte) error }\n\ntype outerM332 struct{ in carM332 }\n\nfunc runOuterM332(o outerM332, a, b []byte) error { return o.in.cb(a, b) }\n\ntype m332 struct{}\n\nfunc (s *m332) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\to := outerM332{carM332{fn}}\n\tout := make([]byte, 4096)\n\treturn runOuterM332(o, out, out)\n}\nfunc (s *m332) TargetTxn() uint64                        { return 0 }\nfunc (s *m332) PageLimit() uint64                        { return 0 }\nfunc (s *m332) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m332) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m332) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m332) Update(uint32, func([]byte) error) error  { return nil }\n\n\nfunc liarProbeM332() error {\n\tvar s0 tree.Store = &m332{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P333: method-value launch of a bound carrier launder", desc: "CopyPage(src, dst, fn) implementations launching a bound carrier's method VALUE (h := car{cb: fn}; runCbM(h.run, out, out)): the method-body invocation of the bound field with call-site byte arguments must require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m333.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM333 struct{ cb func(a, b []byte) error }\n\nfunc (h *carM333) run(a, b []byte) error { return h.cb(a, b) }\n\nfunc runCbM333(f func(a, b []byte) error, a, b []byte) error { return f(a, b) }\n\ntype m333 struct{}\n\nfunc (s *m333) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\th := carM333{cb: fn}\n\tout := make([]byte, 4096)\n\treturn runCbM333(h.run, out, out)\n}\nfunc (s *m333) TargetTxn() uint64                        { return 0 }\nfunc (s *m333) PageLimit() uint64                        { return 0 }\nfunc (s *m333) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m333) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m333) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m333) Update(uint32, func([]byte) error) error  { return nil }\n\n\nfunc liarProbeM333() error {\n\tvar s0 tree.Store = &m333{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P334: delegated address-taken alias chain launder", desc: "CopyPage(src, dst, fn) implementations delegating the formal to a private method whose address-taken local alias chain (cb := fn; p := &cb; cb2 := cb) launches the callback with owned buffers: the delegated chain must stay visible through the address-taken record and fail at the invocation", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m334.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\nfunc runCbM334(f func(a, b []byte) error, a, b []byte) error { return f(a, b) }\n\ntype m334 struct{ buf []byte }\n\nfunc (s *m334) launder(fn func(src, output []byte) error) error {\n\tcb := fn\n\tp := &cb\n\t_ = p\n\tcb2 := cb\n\treturn runCbM334(cb2, s.buf, s.buf)\n}\n\nfunc (s *m334) TargetTxn() uint64                        { return 0 }\nfunc (s *m334) PageLimit() uint64                        { return 0 }\nfunc (s *m334) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m334) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m334) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m334) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m334) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\treturn s.launder(fn)\n}\n\nfunc liarProbeM334() error {\n\tvar s0 tree.Store = &m334{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P335: method value bound to a local then launched launder", desc: "CopyPage(src, dst, fn) implementations with h := car{cb: fn}; mv := h.run; runCbM(mv, out, out): a method VALUE bound to a local and launched with owned buffers must resolve to the bound carrier field and require mapped views", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m335.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM335 struct{ cb func(a, b []byte) error }\n\nfunc (h *carM335) run(a, b []byte) error { return h.cb(a, b) }\n\nfunc runCbM335(f func(a, b []byte) error, a, b []byte) error { return f(a, b) }\n\ntype m335 struct{}\n\nfunc (s *m335) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\th := carM335{cb: fn}\n\tmv := h.run\n\tout := make([]byte, 4096)\n\treturn runCbM335(mv, out, out)\n}\nfunc (s *m335) TargetTxn() uint64                        { return 0 }\nfunc (s *m335) PageLimit() uint64                        { return 0 }\nfunc (s *m335) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m335) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m335) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m335) Update(uint32, func([]byte) error) error  { return nil }\n\n\nfunc liarProbeM335() error {\n\tvar s0 tree.Store = &m335{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P336: mapped mint into an unrelated carrier field callee", desc: "CopyPage(src, dst, fn) implementations passing a CONCRETE mapped page into a helper whose carrier field holds an UNRELATED function (runCar(car{cb: nop}, v)): no store summary binds that carrier key, so the unprovable field body must not be blessed by module-wide carrier suppression; the mapped view must fail", expectFail: true, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m336.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM336 struct{ cb func(a, b []byte) error }\n\nfunc runCarM336(c carM336, p []byte) error { return c.cb(p, p) }\n\nfunc nopM336(a, b []byte) error { return nil }\n\ntype m336 struct{ r *ImmutableReader }\n\nfunc (s *m336) TargetTxn() uint64                        { return 0 }\nfunc (s *m336) PageLimit() uint64                        { return 0 }\nfunc (s *m336) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m336) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m336) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m336) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m336) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tv, perr := s.r.page(0)\n\tif perr != nil {\n\t\treturn perr\n\t}\n\treturn runCarM336(carM336{cb: nopM336}, v)\n}\n\nfunc liarProbeM336() error {\n\tvar s0 tree.Store = &m336{}\n\t_ = s0.CopyPage(0, 1, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P337 benign: local passthrough with mapped views passes", desc: "CopyPage(src, dst, fn) implementations tracing the formal through a local func-var passthrough (g := passthrough; cb := g(fn)) and invoking cb(x, y) with MAPPED views: the sanctioned store callback receives mapped views by contract, so the honest twin must not be over-rejected by the unproven-callee transfer fence", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m337.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\nfunc passthroughM337(f func(src, output []byte) error) func(src, output []byte) error { return f }\n\ntype m337 struct{ r *ImmutableReader }\n\nfunc (s *m337) TargetTxn() uint64                        { return 0 }\nfunc (s *m337) PageLimit() uint64                        { return 0 }\nfunc (s *m337) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m337) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m337) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m337) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m337) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tg := passthroughM337\n\tcb := g(fn)\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn cb(x, y)\n}\n\nfunc honestProbeM337(x *ImmutableReader, src, dst uint32) error {\n\tvar s0 tree.Store = &m337{r: x}\n\t_ = s0.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P338 benign: nested carrier round-trip with mapped views passes", desc: "CopyPage(src, dst, fn) implementations with o := outer{in: car{cb: fn}} then runOuter(o, x, y) with MAPPED views: the nested-carrier fence must see the mapped byte arguments and accept the honest twin", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m338.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\ntype carM338 struct{ cb func(src, output []byte) error }\n\ntype outerM338 struct{ in carM338 }\n\nfunc runOuterM338(o outerM338, a, b []byte) error { return o.in.cb(a, b) }\n\ntype m338 struct{ r *ImmutableReader }\n\nfunc (s *m338) TargetTxn() uint64                        { return 0 }\nfunc (s *m338) PageLimit() uint64                        { return 0 }\nfunc (s *m338) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m338) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m338) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m338) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m338) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\to := outerM338{in: carM338{cb: fn}}\n\treturn runOuterM338(o, x, y)\n}\n\nfunc honestProbeM338(x *ImmutableReader, src, dst uint32) error {\n\tvar s0 tree.Store = &m338{r: x}\n\t_ = s0.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
	{name: "P339 benign: delegated address-taken and method-value launches with mapped views pass", desc: "CopyPage(src, dst, fn) implementations reaching the callback through a delegated address-taken launder chain (cb := fn; p := &cb; cb2 := cb; runCb(cb2, x, y)) or a method-value launch (h := car{cb: fn}; runCb(h.run, x, y)) with MAPPED views: both honest twins must not be over-rejected", expectFail: false, ops: []batteryOp{
		batteryOp{kind: "create", path: "internal/reader/gatemut_m339.go", content: "package reader\n\nimport \"github.com/firehol/iprange/v4/go/internal/tree\"\n\nfunc runCbM339(f func(a, b []byte) error, a, b []byte) error { return f(a, b) }\n\ntype carM339 struct{ cb func(a, b []byte) error }\n\nfunc (h *carM339) run(a, b []byte) error { return h.cb(a, b) }\n\ntype m339a struct{ r *ImmutableReader }\n\nfunc (s *m339a) launder(fn func(src, output []byte) error, x, y []byte) error {\n\tcb := fn\n\tp := &cb\n\t_ = p\n\tcb2 := cb\n\treturn runCbM339(cb2, x, y)\n}\n\nfunc (s *m339a) TargetTxn() uint64                        { return 0 }\nfunc (s *m339a) PageLimit() uint64                        { return 0 }\nfunc (s *m339a) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m339a) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m339a) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m339a) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m339a) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\treturn s.launder(fn, x, y)\n}\n\ntype m339b struct{ r *ImmutableReader }\n\nfunc (s *m339b) TargetTxn() uint64                        { return 0 }\nfunc (s *m339b) PageLimit() uint64                        { return 0 }\nfunc (s *m339b) Inspect(uint32, func([]byte) error) error { return nil }\nfunc (s *m339b) Allocate() (uint32, error)                { return 0, nil }\nfunc (s *m339b) DiscardPrivate(uint32) error              { return nil }\nfunc (s *m339b) Update(uint32, func([]byte) error) error  { return nil }\n\nfunc (s *m339b) CopyPage(source, destination uint32, fn func(src, output []byte) error) error {\n\tx, err := s.r.page(source)\n\tif err != nil {\n\t\treturn err\n\t}\n\ty, err := s.r.page(destination)\n\tif err != nil {\n\t\treturn err\n\t}\n\th := carM339{cb: fn}\n\treturn runCbM339(h.run, x, y)\n}\n\nfunc honestProbeM339(x *ImmutableReader, src, dst uint32) error {\n\tvar s0 tree.Store = &m339a{r: x}\n\tvar s1 tree.Store = &m339b{r: x}\n\t_ = s0.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\t_ = s1.CopyPage(src, dst, func(a1, b1 []byte) error { copy(b1, a1); return nil })\n\treturn nil\n}\n"},
	}},
}
