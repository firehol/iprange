...
func TestScopeRegistryHashMapIntern(t *testing.T) {
	...
	id1 := reg.Intern([]byte{0x01})
	id2 := reg.Intern([]byte{0x03})
	id1b := reg.Intern([]byte{0x01})
...
func TestScopeRegistryFromEntriesBuildsIndex(t *testing.T) {
	...
	id := reg.Intern([]byte{0xAB})
	if id != 1 {
		t.Fatalf("expected id=1, got %d", id)
	}
	id = reg.Intern([]byte{0xCD})
	if id != 2 {
		t.Fatalf("expected id=2, got %d", id)
	}
	// New bitmap gets a new ID.
	id = reg.Intern([]byte{0xEF})
	if id != 3 {
...
func TestMigrateRandomOracle(t *testing.T) {
	...
	for trial := 0; trial < 30; trial++ {
		...
		// Build expected map (desired last-wins).
		expected := make(map[uint32]uint32)
		sortedDesired := make([]DesiredRecord[Ipv4Key], len(desired))
		copy(sortedDesired, desired)
		sortByFrom(sortedDesired)
