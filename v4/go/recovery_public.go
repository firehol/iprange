...
// recover_offline / recover_live, inspect_recovery_candidates, and the
// recovery re-exports): one bounded recovery construction of an exact
// retained candidate into a fresh published output. The machine mirrors
// iprange-livedb/src/recovery/api.rs plus the worker client create
// position: the budget preflight, the fail-if-exists destination
// attempt, the source open per mode with the attempt discard on
// failure, the source-identity proof, the kind-split build, the final
// source check, and the publication terminal. The offline
// certification, the live support refusal, and the live newest-only
// candidate rule mirror the Rust api arms exactly.
...
// inspect_recovery_candidates). cancellation, when non-nil, is checked
// between bounded steps; the inspection never scans a page graph.
func InspectRecoveryCandidates(path string, mode RecoveryInspectionMode, budget *ValidationBudget, cancellation *CancellationToken) (*RecoveryCandidateInspection, error) {
	result, err := recovery.InspectRecoveryCandidates(path, mode, budget, cancellation.check)
	if err != nil {
...
func RecoverImmutable(sourcePath string, candidate *RecoveryCandidate, destinationPath string, budget *RecoveryBudget, sink RecoverySink, cancellation *CancellationToken) (*RecoveryResult, *RecoveryPreparationFailure) {
	...
	result, failure := recovery.RecoverImmutable(sourcePath, candidate, destinationPath, budget, internalCheck(cancellation), sink)
...
func RecoverOffline(sourcePath string, candidate *RecoveryCandidate, destinationPath string, certification OfflineQuiescenceCertification, budget *RecoveryBudget, sink RecoverySink, cancellation *CancellationToken) (*RecoveryResult, *RecoveryPreparationFailure) {
	...
	result, failure := recovery.RecoverOffline(sourcePath, candidate, destinationPath, budget, internalCheck(cancellation), sink)
	return result, publicRecoveryFailure(failure)
}
...
func RecoverLive(sourcePath string, candidate *RecoveryCandidate, destinationPath string, budget *RecoveryBudget, sink RecoverySink, cancellation *CancellationToken) (*RecoveryResult, *RecoveryPreparationFailure) {
	...
	result, failure := recovery.RecoverLive(sourcePath, candidate, destinationPath, budget, internalCheck(cancellation), sink)
	return result, publicRecoveryFailure(failure)
}
