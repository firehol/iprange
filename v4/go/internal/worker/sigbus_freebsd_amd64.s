...
// Raw FreeBSD amd64 syscall numbers (sys/syscall.h, FreeBSD 14; the
// 64-bit ABI numbers: the pre-12 compat entries are not used).
...
// DX=ucontext. It never calls Go. Register contract (FreeBSD preserves
...
// the kernel frames every return through the process sigtramp, so the
...
TEXT ·sigbusHandler(SB), NOSPLIT|NOFRAME, $0-0
	...
	TESTQ CX, CX
	JZ chain
	MOVQ SI_ADDR_OFF(R13), R15 // si_addr (offset 24 on FreeBSD)
	MOVQ R15, AX
	SUBQ CTL_BASE(R8), AX
	JC chain
	CMPQ AX, CX
...
chain:
	...
	// chained handler returns to the process sigtramp and the kernel
...
// action into the 16-byte slot at 32(SP) (posix.rs apply_mask): the
...
TEXT ·chain_mask(SB), NOSPLIT|NOFRAME, $0-0
	...
	CMPQ AX, $0
	JNE mask_ret
	MOVQ ·previousAction+SACT_MASK(SB), BX // previous mask low half
	ORQ BX, 32(SP)
	MOVQ ·previousAction+SACT_MASK+8(SB), BX // previous mask high half
	ORQ BX, 40(SP)
	MOVQ ·previousAction+SACT_FLAGS(SB), BX // flags
	TESTQ $SA_NODEFER, BX
	JNZ mask_nodefer
