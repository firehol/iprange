...
// the kernel frames every handler return through the thread sigtramp,
// so the chained handler returns restore the interrupted context
// directly. Struct layouts and syscall numbers are the macOS ABI
// (verified against the SDK headers on arm64 and x86-64).
...
// darwinSigaction mirrors the macOS sigaction struct (handler@0,
// mask@8 as a 32-bit sigset, flags@12; 16 bytes).
...
func (c *Control) InstallHandler() (*Handler, error) {
	...
	selectedAction := darwinSigaction{
