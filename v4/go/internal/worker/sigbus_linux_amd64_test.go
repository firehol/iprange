...
// fault (Rust worker/posix.rs tests): the owned-fault record survives the
// worker exit, and an unarmed fault chains into the Go runtime's own
// SIGBUS handler without a hang and without exiting 197. The 15-case
// previous-disposition matrix lives in sigbus_matrix_v4work_test.go and
...
func signalMapping(t *testing.T, label string, truncate bool) (*mapping.Mapping, uintptr, uint64) {
	...
	native := uint64(unix.Getpagesize())
...
// raise() (tgkill against the calling thread's tid).
func raiseSigbus() {
