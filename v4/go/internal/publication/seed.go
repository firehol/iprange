...
func captureSeed(output *preparedOutput) seed {
	...
	return seed{
		...
		destinationBasename:   platformEncodedBytes(mainName),
		...
		privateOutputBasename: platformEncodedBytes(name),
		names: seedNames{
			privateOutput:      platformEncodedBytes(name),
			privateReservation: platformEncodedBytes(reservation),
			coordination:       platformEncodedBytes(d.coordinationName()),
		},
	}
}
...
func reconstructSeed(destination *destination, header reservationHeader) (seed, error) {
	...
	return seed{
		...
		privateOutputBasename: platformEncodedBytes(privateOutput),
		names: seedNames{
			privateOutput:      platformEncodedBytes(privateOutput),
			privateReservation: platformEncodedBytes(privateReservation),
			coordination:       platformEncodedBytes(destination.coordinationName()),
		},
	}, nil
}
