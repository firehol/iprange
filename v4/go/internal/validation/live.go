...
func sweepLiveSelected(source *live.LiveSource, budget *ValidationBudget, check func() error, sink ValidationSink) (*ValidationResult, *ValidationFailure) {
	...
	// Rust validation.rs:218 probe_source (validate_live_selected arm):
	// the sweep runs inside the armed Source probe of the claimed live
	// mapping.
	scanErr := source.Mapping().Probe(mapping.RoleSource, func() error {
