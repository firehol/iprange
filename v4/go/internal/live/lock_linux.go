...
// Linux implements the sidecar byte-range locks with open-file-description
// fcntl locks (Rust live_lock linux platform): F_OFD_SETLK for try,
// F_OFD_SETLKW for blocking wait, F_UNLCK for release. The lock ranges
// are one byte at the caller-supplied offset; the kernel releases them
// when the last descriptor of the open file description closes.
...
func unlockOFD(f *os.File, offset uint64) error {
	...
	for {
		if err := unix.FcntlFlock(f.Fd(), unix.F_OFD_SETLK, &fl); err == nil {
			return nil
		} else if errors.Is(err, unix.EINTR) {
			continue
		} else {
...
// requireLiveSupported permits live coordination on linux (OFD
