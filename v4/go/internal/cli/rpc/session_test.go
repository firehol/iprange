...
func TestIdlessNonCancelIsInvalidNotification(t *testing.T) {
	// Spec (D3, wave-10): an id-less non-cancel request is an
	// invalid notification: not executed, answered with one -32600
	// whose id is null. Inside a batch, the whole batch is rejected
	// with one -32600 id:null and healthy siblings are not executed.
	out, err := runService(t,
		`{"jsonrpc":"2.0","method":"iprange.v1.system.describe","params":{}}`+"\n")
	if err != nil {
		t.Fatalf("run: %v", err)
	}
	got := lines(out)
	if len(got) != 1 ||
		!strings.Contains(got[0], `"code":-32600`) ||
		!strings.Contains(got[0], `"id":null`) {
		t.Fatalf("id-less single: output = %q", out)
	}
	batch := `[{"jsonrpc":"2.0","id":"ok1","method":"iprange.v1.system.describe","params":{}},` +
		`{"jsonrpc":"2.0","method":"iprange.v1.system.describe","params":{}},` +
		`{"jsonrpc":"2.0","id":"ok2","method":"iprange.v1.system.describe","params":{}}]` + "\n"
	out, err = runService(t, batch)
	if err != nil {
	...
	if len(got) != 1 ||
		!strings.Contains(got[0], `"code":-32600`) ||
		!strings.Contains(got[0], `"id":null`) {
		t.Fatalf("id-less in batch: output = %q", out)
	}
}
