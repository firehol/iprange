...
func TestNulTextRecordsResolvedByC(t *testing.T) {
	if _, err := os.Stat(cReference); err != nil {
		t.Skipf("no C reference at %s: the resolver-dependent cases cannot be judged", cReference)
	}
	dir := t.TempDir()
	writeNulFixtures(t, dir)
