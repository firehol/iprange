...
// SID-based creator profile (Rust security/windows.rs) is a tracked SOW-0026
...
// capture lands with the SOW-0026 surface; every policy operation refuses
...
// the SID machinery, tracked with the SOW-0026 live surface.
func SecureCreatorOnly(*os.File, *Profile) error {
	return unsupported("creator-only access policy requires the Windows SID machinery, tracked with the SOW-0026 live surface")
}

// CreatorOnlyCommitment refuses on Windows (Rust
// creator_only_commitment Windows arm, SOW-0026).
func CreatorOnlyCommitment(*os.File) ([32]byte, error) {
	return [32]byte{}, unsupported("creator-only access policy requires the Windows SID machinery, tracked with the SOW-0026 live surface")
}
