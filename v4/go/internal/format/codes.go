...
// responses). The wire decoder validates preserved evidence with it and
// the CLI adapter codes error objects through it.
var errorCodeWireNames = map[ErrorCode]string{
...
// errorCodeFromWireNames is the reverse of errorCodeWireNames, built
// once at init for O(1) wire-name resolution.
var errorCodeFromWireNames = func() map[string]ErrorCode {
	reverse := make(map[string]ErrorCode, len(errorCodeWireNames))
...
func ErrorCodeFromWireName(name string) (ErrorCode, bool) {
	code, ok := errorCodeFromWireNames[name]
	return code, ok
}
