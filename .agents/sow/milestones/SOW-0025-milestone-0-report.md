...
- The old tree has no conformance coupling (no `cases.json` / `.iprdb` fixture
  ...
  and a rejected positional-I/O + in-heap-page architecture (4 files with
  content-transfer calls, 24 files with complete 4 KiB page arrays).
- Exactly six production files carry scalar logic worth transferring after
  literal-vector verification: `errors.go`, `key.go`, `cardinality.go`,
  `page.go`, `name_binding.go`, `process_identity.go` (plus five test files).
  Everything else is proposed for deletion.
- Projected pure-Go production size: roughly 32–44k newline-counted lines
  (~37k midpoint) for full Phase-1 parity — about 40–53% of the accepted Rust
  implementation (82,516 production lines in `iprange-livedb` alone). The
  directional 5k target will be exceeded about 7x; reported honestly below.
- Two decisions are requested from the user at the end of this report: the
  exact deletion set (98 tracked files) and the fault-worker native-boundary
  policy.
...
- Public surface: 227 package-level exports total, all in 7 files
  (`errors.go` 68, `contract.go` 45, `bootstrap.go` 41, `types.go` 22,
  `page.go` 21, `cardinality.go` 16, `key.go` 7). 43 of 50 production files
  export nothing.
...
- Wire constants: current magics everywhere (`IPRANGE4`, `IP4P`, `IPR4RSV1`
  `IPR4RDRS`), zero v3 references anywhere in `v4/go`.
...
- Error codes: `errors.go` defines codes 1–64 whose names match the current
  Rust codes 1–64 exactly; codes 65–69 (FaultWorkerUnavailable,
  FaultWorkerFailed, UnsupportedStructure, WrongStructureKind,
  StructureIdExhausted) are missing.
- Prohibited architecture evidence: content-transfer calls in 4 files
  (`os_linux.go` ReadAt/WriteAt/Truncate/Sync, `page_source_linux.go` `unix.Pread`,
  `page_source_other.go` `File.ReadAt`, `live_writer_linux.go` Truncate/Sync);
  complete-page arrays in 24 files (e.g. `private_page_pool.go:99`,
  `bitmap_cow.go:261-262`, `retirement_writer.go` 8 sites,
  `os_linux.go:544` two whole meta pages). Everything reads/writes through a
...
- Sidecar/reservation layouts are stale: `sidecar.go` magic `IPR4RDRS` (spec:
  `IPRDRS4\0`, header size 68, 16-byte slots) and `sidecar_transition.go`
  uses 64-byte slot images (spec: 16-byte dual-txn-complement slots).
...
- `binary-format-v4.md` (4,105 lines) fully inventoried into 48 numbered
  ...
  conformance). `design-iprange-engine.md` (530 lines) inventoried into 16
...
| 4 | `ErrorCode` 1–69, `Error`, `Result` | `ErrorCode uint32` 1–69 (1–64 transfer, add 65–69), `Error{Code,Detail,Cause}`; Go `error` | root | T+R |
...
validates; budgets everywhere; `process_identity` and `name_binding` land in
`internal/live`/`internal/format` as fresh code (concepts transfer, files do
not).
...
|---|---|---|
| `errors.go` (root) | T | Codes 1–64 match current Rust names exactly; extend with 65–69. No I/O, no page arrays. |
...
| `key_test.go`, `cardinality_test.go`, `page_test.go`, `name_binding_test.go`, `process_identity_test.go` | T | Cover only transferable scalar logic; re-verify. |
...
| `process_identity.go` | T | Self-contained `/proc/<pid>/stat` field-22 parser with its own test; not an SDK artifact, no page arrays; re-verify field derivation against new worker/live design needs. |
...
| `sidecar.go`, `sidecar_transition.go`, `reservation.go` | D | Stale layouts: magic `IPR4RDRS` vs spec `IPRDRS4`, 64-byte slot images vs spec 16-byte slots. |
| `live_reader_linux.go`, `live_writer_linux.go`, `os_linux.go` | D | Linux-only positional I/O + flock architecture (`ReadAt`/`WriteAt`/`Truncate`/`Sync`, whole meta pages in stack). |
...
| remaining 54 test files | D | Tied to deleted internals; behavioral models (arrival-order normalization, codec vectors, crash points) are re-created in new tests. |

### 6.2 Same-failure / duplicate-authority searches

- Content-transfer calls: 4 files; complete-page arrays: 24 files; stale-format
  constants: `retention` ×2 files, 1 MiB limit ×1, v3 ×0; structured/threat/
  NetworkEnrichment references ×0; conformance/corpus references in any Go
  file ×0. Each class is searched repository-wide in the future port gates,
  not just at the cited examples.
...
Proposed tracked deletions: **98 files** = 44 production + 54 test files.

- Production (44): every §6.1 file marked D — `contract.go`, `bootstrap.go`,
  root `types.go`, `doc.go`; `page_source.go`, `page_source_linux.go`,
...
- Tests (54): every `*_test.go` tracked file except the five transfer tests —
  ...
  `private_page_pool_test.go`, `race_disabled_test.go`,
...
- Kept tracked (13): `go.mod`, `go.sum`, `errors.go`, `key.go`,
  `cardinality.go`, `page.go`, `name_binding.go`, `process_identity.go` +
  their five tests. These are transfer candidates: content is re-verified
  against literal vectors and the current contract in Milestone 1 *before* it
  is used; paths may move into the new package layout.
...
8. **Error/reason-code drift.** 69 error codes, 47 validation reasons, 17
   objects must stay 1:1 with Rust; one table per language + literal-vector
   tests, both cross-checked in conformance.
...
## 9. Projected size (honest estimate)
...
**Decision 1 — deletion set.** Approve deleting the exact 98 tracked files
listed in §7 (44 production + 54 test), keeping `go.mod`/`go.sum` and the 13
transfer files (6 production + 5 test + module files), whose content is
re-verified in Milestone 1 before reuse?
- A. Approve the full list (recommended: the files are obsolete
  architecture; nothing else in the repo references them; Rust conformance
  suite is unaffected — verified: zero Go references to the corpus; the
  transfer content is preserved and re-verified before use).
- B. Approve with exceptions (name them).
- C. Hold: delete nothing until Milestone 1 verification evidence exists for
  the transfer files (deletion then happens in the same commit as their
  verified re-creation).
...
proceed?
- A. Prove pure-Go first; if disproven by evidence, use a minimal
  project-owned assembly shim and document it in the milestone report
  (recommended: bounded pre-decision, evidence still reviewed, no silent
  boundary addition).
- B. Pure-Go only: if impossible, stop (per the SOW's default contract).
- C. Decide later, after the Milestone 1 feasibility evidence.

## 11. Milestone 1 readiness

Safe to start. Milestone 0 produced zero production edits; the repository is
clean at `972960a`; baseline gates pass; the target (specs + accepted Rust
behavior + corpus) and the exit criteria (transfer-file verification,
mapping/lifetime owner, immutable reader cross-opening all five Rust fixtures,
malformed bootstrap rejection, zero-allocation lookup evidence, worker
feasibility report, Decision 1/2 answers) are fully specified.
