...
an alternate signal stack, saves the complete previous `sigaction` exposed by
the platform, and arms one exact
`(mapping_generation, mapping_role, base, length)` probe at a time. It may claim
a signal only when all of these are true:
...
Darwin does not return an installed `SA_RESETHAND` bit through `sigaction`, even
though it applies that bit internally. The worker cannot infer an unobservable
one-shot flag from a handler address. It preserves every field Darwin exposes,
calls a custom predecessor with the correct ABI and mask, and makes no guess
about that hidden bit. This limitation affects only a custom one-shot `SIGBUS`
handler preinstalled inside the dedicated recovery worker; owned SDK faults and
default, ignored, persistent custom, masked, and `SA_NODEFER` dispositions keep
the normal contract.
