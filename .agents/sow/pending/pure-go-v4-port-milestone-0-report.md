...
| 26 | `NetworkEnrichmentV1`, `NetworkEnrichmentV1Location`, `NetworkEnrichmentV1View` (`value()`, `threat_membership()`), cursors/ranges | same types; `Location NetworkEnrichmentV1Location` + `HasLocation` (decision 5A, ratified 2026-08-13: the zero-allocation equivalent of Rust's `Option<NetworkEnrichmentV1Location>`; supersedes the earlier pointer spelling); `View.Value()` + `ThreatMembership()` | root + `internal/structured` codec | N |
