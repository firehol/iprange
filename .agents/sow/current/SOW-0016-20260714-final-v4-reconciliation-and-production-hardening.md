...
### 2026-07-27 - Rust C-ABI implementation decomposition

Evidence and constraint:

- The frozen generation-1 contract contains 159 exact manifest entities and explicitly
  forbids a transaction-handle family. One writer handle owns transaction,
  workflow, cancellation, child-reference, commit, and abort state.
- Native Rust operation objects currently borrow `&mut LiveWriter`. A C handle
  must survive across calls and therefore cannot safely store one of those
  self-referential borrowed objects beside its writer.
- Buffering input until `FinishInput` would violate bounded memory and the
  required direct-to-private-COW arrival-order normalization. Reimplementing
  mutations in the ABI would create a second semantic engine and drift risk.

Implementation decomposition:

1. Extract only the small borrow-free operation state from the existing native
   Rust facades. Keep all mutation, normalization, workflow comparison, commit,
   abort, and cleanup logic in the existing engine. Native Rust facades and the
   C binding call those same state transitions.
2. Add a separate `iprange-capi` workspace crate producing the static and
   dynamic Rust-provided C library. It owns pointer checking, fixed ABI layouts,
   path/IP conversion, panic containment, fail-fast non-reentrant gates, child
   lifetime accounting, callbacks, opaque errors/reports, and exported symbols;
   it contains no page, tree, allocator, or format logic.
3. The opaque C writer owns exactly one operation-state enum. Feed references,
   membership references, and builders remain operation-bound children and
   block writer close until destroyed. Reader cursors/views retain a parent
   child claim. Reader point lookup remains free of a per-call lock or active
   counter as required by the specification.
4. Generate the checked C/C++ header and machine-readable ABI manifest from one
   deterministic binding schema. Tests compare regenerated bytes, reject missing
   or extra frozen symbols, compile C and C++ layout assertions, inspect actual
   library exports, and link native C behavior tests.
5. Implement in bounded checkpoints: ABI values/errors/gates; reader/cursor/view
   surface; writer advanced and high-level workflows; lifecycle and terminal
   reports; validation/recovery/snapshot/maintenance; then native integration
   and complete export/prototype verification.

Risk controls:

- The state extraction is behavior-preserving and remains covered by the full
  native Rust suite before ABI behavior is added.
- Every boundary export catches unwind; allocator abort/OOM remains explicitly
  non-recoverable. Pointer, alignment, length, enum, family, reserved-zero, and
  callback checks precede engine mutation.
- No v4 bytes, validation default, sorting architecture, temporary-file policy,
  native public semantics, Go code, signing, or Phase-2 algebra changes in this
  checkpoint.

Derived contract reconciliation:

- `CreateLive` creates the canonical main and canonical sidecar directly with
  exclusive creation. It has no private create-temp naming class, no list
  operation, and no generic temp-removal authority; interrupted creation is
  resolved from its factual `CreateResult`. The two unimplemented
  `list_abandoned_create_temps` / `remove_abandoned_create_temp` C symbols were
  therefore dead API with no corresponding Rust operation or durable artifact.
  They were removed from the unshipped generation-1 manifest instead of
  inventing a temporary file or a no-op API.
- User decision 73 and the binary-format contract define six cleanup artifact
  kinds. The C registry had accidentally omitted `OwnedMain` and assigned value
  5 to `UnpublishedMainTail`. It now records `OwnedMain=5` and
  `UnpublishedMainTail=6`, matching failed-creation cleanup and Windows GC
  evidence.

### 2026-07-27 - frozen Rust C-ABI milestone

Implementation:

- Added the separate `iprange-capi` crate as a thin adapter over the one Rust
  engine. Its frozen generation-1 surface contains 136 functions, 12 opaque
  handle kinds, 11 callback types, 53 fixed structures, and 292 numeric
  constants. It contains no page, tree, allocator, normalization, or format
  implementation.
- The core exposes one hidden borrow-free ownership bridge for C-held reader,
  cursor, writer, workflow, feed, and membership state. Both native Rust
  facades and the C adapter continue to call the same mutation and commit
  implementation; input is applied directly to private COW pages in arrival
  order and is never buffered into a second feed representation.
- Every export clears and validates its declared outputs, checks boundary
  shapes and handle kinds, contains Rust panics, and returns an owned typed
  error. Fail-fast gates protect caller-serialized state and reject writer
  callback reentry. Reader point lookups retain the required lock-free ABI
  path.
- Generated `iprange_v4.h` and the machine-readable ABI manifest are committed
  and checked byte-for-byte. Native C and C++ compilation proves every fixed
  layout; shared-library symbol inspection rejects missing or extra exports;
  five linked C programs exercise every frozen function across reader, writer,
  membership, workflow, lifecycle, validation, recovery, snapshot,
  maintenance, cleanup, malformed-argument, callback, and panic paths.
- Publication-residue removal exposed one core ownership defect: a failure
  after authoritative unlink could report a cleanup guard while dropping the
  exact retained directory and file handles required to finish it. Removal now
  returns a move-only retry handle containing that exact authority, and the C
  report owns it until explicitly taken. A permanent injected failure test
  proves retry resumes finalization without reopening authority by pathname.
- Final boundary review found two related cause-preservation defects. Nested
  original causes were overwritten when cleanup also failed, and a C source
  callback's caller code/message was lost on failed draft cleanup. The adapter
  now retains the complete original chain followed by the cleanup cause, with
  permanent tests for both cases.
- Header generation parses the binding source directly. This preserves
  deterministic generation on Rust 1.74 without asking old Cargo to parse
  irrelevant target-only packages whose manifests require a newer edition.

Validation:

- Current Rust and Rust 1.74.1 each pass the complete all-feature workspace
  matrix: 426 test functions, 424 passed and two intentional subprocess entry
  points ignored.
- Strict workspace/all-feature/all-target Clippy, formatting, warning-denied
  rustdoc, generated-header/manifest equality, C11/C++17 layout compilation,
  exact shared-library exports, and all five native C behavior programs pass.
- Nightly AddressSanitizer with leak detection passes all 13 C-ABI unit tests
  and all 310 core unit tests; the same two subprocess entry points are
  intentionally ignored. Valgrind reports no definite or indirect leak in any
  of the five native C programs. A native C workflow also links and runs
  successfully against the Rust static archive.
- Warning-free all-target checks pass for Windows GNU, Apple ARM, and FreeBSD.
  Windows core code generation also passes. Native Windows, macOS, and FreeBSD
  process/crash execution remains outside this local milestone because no
  external test system was used without user authorization.
- The C boundary is complete, but this is not Rust acceptance or SOW closure.
  Current-format conformance artifacts, final local release/resource gates,
