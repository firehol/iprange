...
below); the round-9 re-review returned fourteen further gate findings
(Luna), all probe-verified and fixed with durable battery pins
(round-10 entry below); milestone close is pending completion of that
round (six-resident swarm re-approval, then sol, per the user review
...
mmap gate is an 12,619-line typed toolchain (12,067-line go/types module
...
round-25 entry below. The durable battery is 551 cases (477 rejections,
74 benign acceptances) plus 9 shell environment mutations and passes end
to end.
...
- P1 enforcement - fixed: the type-light 4,738-line AST taint engine and
  ...
  (551 cases: 477 rejections, 74 benign); the shell harness keeps only the import-boundary, module-graph,
...
the import-graph gate with its 551-case battery, and the SOW audit.
...
lines; gate tooling 12,619 raw lines total (12,067 go-gate + 552 shell). Hot-path benchmarks on the
...
- `./check-import-graph.sh` — passes; the content-transfer scan is the typed
  ...
  exempted as exact, file-taint-verified shapes; the 551-form `--self-test`
