...
fn round_trip_random_disjoint_ranges() {
    for seed in 0..40u64 {
        ...
        for &(s, e, _) in &spec {
            assert!(
                r.lookup_v4(Ipv4Key(s)).unwrap().is_some(),
                "seed {seed}: start {s} missing"
            );
            assert!(
                r.lookup_v4(Ipv4Key(e)).unwrap().is_some(),
                "seed {seed}: end {e} missing"
            );
            let mid = s + (e - s) / 2;
            assert!(
                r.lookup_v4(Ipv4Key(mid)).unwrap().is_some(),
                "seed {seed}: mid {mid} missing"
            );
        }
